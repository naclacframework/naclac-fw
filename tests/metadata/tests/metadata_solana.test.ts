...
import { MetadataSolanaClient } from "../clients/src/generated/metadata_solana";

describe("Naclac MetadataSolana Test Suite", () => {
  let client: MetadataSolanaClient;
  let payer: naclac.KeyPairSigner;
  let counterPda: naclac.Address;

  before(async () => {
    payer = await naclac.loadNodeWallet();
    client = new MetadataSolanaClient("localnet", payer);

...
/// Adds the accounts passed via `remaining_accounts` (Collection accounts)
/// to a Group via `naclac_metadata::add_collections_to_group_signed`.
#[derive(Accounts)]
pub struct AddCollectionsToGroup {
    #[account(mut)]
    pub group: Signer,
...
pub fn add_collections_to_group(ctx: Context<AddCollectionsToGroup>) -> Result {
    ...
    add_collections_to_group_signed(

...
/// Removes the accounts passed via `remaining_accounts` (child `GroupV1`
/// accounts) from a parent Group via
/// `naclac_metadata::remove_groups_from_group_signed`.
#[derive(Accounts)]
...
pub fn remove_groups_from_group(ctx: Context<RemoveGroupsFromGroup>) -> Result {
    ...
    remove_groups_from_group_signed(

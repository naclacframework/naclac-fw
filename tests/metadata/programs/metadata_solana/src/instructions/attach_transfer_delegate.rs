...
/// Attaches a TransferDelegate plugin to an existing Asset via
/// `naclac_metadata::attach_transfer_delegate_signed`.
#[derive(Accounts)]
pub struct AttachTransferDelegate {
    #[account(mut)]
    pub asset: Signer,
...
pub fn attach_transfer_delegate(ctx: Context<AttachTransferDelegate>) -> Result {
    attach_transfer_delegate_signed(

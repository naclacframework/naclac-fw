...
/// Attaches an Edition plugin to an existing Asset via
/// `attach_edition_signed`. Real mpl-core rejects this unconditionally
/// (`Edition::validate_add_plugin`) — Edition may only be set at asset
/// creation time, never via a later `AddPluginV1` — so this instruction
/// exists to exercise (and confirm) that rejection, not to succeed.
#[derive(Accounts)]
pub struct AttachEdition {
    #[account(mut)]
    pub asset: Signer,

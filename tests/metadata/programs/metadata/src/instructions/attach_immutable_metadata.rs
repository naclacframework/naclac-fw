...
/// Attaches ImmutableMetadata plugin to an existing Asset via
/// `naclac_metadata::attach_immutable_metadata_signed`.
#[derive(Accounts)]
pub struct AttachImmutableMetadata {
    #[account(mut)]
    pub asset: Signer,
...
pub fn attach_immutable_metadata(ctx: Context<AttachImmutableMetadata>) -> Result {
    attach_immutable_metadata_signed(

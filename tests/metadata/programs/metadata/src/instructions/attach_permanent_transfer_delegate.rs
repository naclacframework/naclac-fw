...
/// Attaches a PermanentTransferDelegate plugin to an existing Asset via
/// `naclac_metadata::attach_permanent_transfer_delegate_signed`.
#[derive(Accounts)]
pub struct AttachPermanentTransferDelegate {
    #[account(mut)]
    pub asset: Signer,
...
pub fn attach_permanent_transfer_delegate(ctx: Context<AttachPermanentTransferDelegate>) -> Result {
    attach_permanent_transfer_delegate_signed(

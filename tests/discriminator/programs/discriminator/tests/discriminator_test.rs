...
fn load_program(provider: &NaclacProvider) {
    ...
    so_path.push("target/deploy/discriminator.so");

    provider
        .add_program(&PROGRAM_ID, so_path.to_str().unwrap())
        .expect("Failed to load discriminator program binary");
}
...
fn rejects_type_confused_account_in_vault_slot() {
    ...
    // used — `accounts.rs`). This is the pinocchio/zero-copy backend, so

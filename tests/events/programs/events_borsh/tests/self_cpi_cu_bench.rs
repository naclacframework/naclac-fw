...
/// zero accounts and a zero-length payload) under the solana-program+borsh
/// backend, via litesvm's actual SVM execution â€” not an estimate. Also runs
/// a same-shaped no-CPI baseline (identical single `address`-constrained
/// account, no invoke) so the fixed CPI overhead can be isolated from the
/// surrounding instruction's own dispatch/validation cost.
#[test]
fn self_cpi_baseline_cu_cost() {
    ...
    println!(
        "[events_borsh] no-CPI baseline: {} CU | self-CPI (empty payload): {} CU (isolated CPI overhead: {} CU) | sol_log_data emit (1 field CounterIncremented): {} CU (isolated emit overhead: {} CU)",

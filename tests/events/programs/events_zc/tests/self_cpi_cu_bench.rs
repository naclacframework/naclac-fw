...
fn self_cpi_baseline_cu_cost() {
    ...
    println!(
        "[events_zc] no-CPI baseline: {} CU | self-CPI (empty payload): {} CU (isolated CPI overhead: {} CU) | sol_log_data emit (1 field CounterIncremented): {} CU (isolated emit overhead: {} CU)",

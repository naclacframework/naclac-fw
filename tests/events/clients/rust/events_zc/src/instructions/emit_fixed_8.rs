...
#[cfg(feature = "cpi")]
pub struct EmitFixed8CpiAccounts<'a> {
    /// SAFETY: only its address is checked; its data is never read.
    pub program: crate::sdk_core_cpi::CpiHandle<'a>,
}

#[cfg(feature = "cpi")]
pub struct EmitFixed8CpiCall<'a> {
    pub accounts: EmitFixed8CpiAccounts<'a>,
    pub signer_seeds: &'a [&'a [&'a [u8]]],
    pub remaining_accounts: &'a [(crate::sdk_core_cpi::AccountInfo, bool, bool)],
}

#[cfg(feature = "cpi")]
pub trait EmitFixed8Cpi<'info> {
    fn emit_fixed_8<'a>(
        &self,
        accounts: EmitFixed8CpiAccounts<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        self.emit_fixed_8_with_remaining_accounts(EmitFixed8CpiCall {
        accounts,
        signer_seeds: &[],
        remaining_accounts: &[],
    })
    ...
    fn emit_fixed_8_with_remaining_accounts<'a>(
        &self,
        call: EmitFixed8CpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()>;

}

#[cfg(feature = "cpi")]
impl<'info> EmitFixed8Cpi<'info> for crate::sdk_core_cpi::Program<crate::EventsZc> {
    fn emit_fixed_8_with_remaining_accounts<'a>(
        &self,
        call: EmitFixed8CpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        let EmitFixed8CpiCall { accounts, signer_seeds, remaining_accounts } = call;
        let mut ix_data = crate::sdk_core_cpi::Vec::new();
        ix_data.extend_from_slice(&[166, 42, 161, 212, 88, 155, 188, 151]);

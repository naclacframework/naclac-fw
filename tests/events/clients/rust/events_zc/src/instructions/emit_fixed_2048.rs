...
#[cfg(feature = "cpi")]
pub struct EmitFixed2048CpiAccounts<'a> {
    /// SAFETY: only its address is checked; its data is never read.
    pub program: crate::sdk_core_cpi::CpiHandle<'a>,
}

#[cfg(feature = "cpi")]
pub struct EmitFixed2048CpiCall<'a> {
    pub accounts: EmitFixed2048CpiAccounts<'a>,
    pub signer_seeds: &'a [&'a [&'a [u8]]],
    pub remaining_accounts: &'a [(crate::sdk_core_cpi::AccountInfo, bool, bool)],
}

#[cfg(feature = "cpi")]
pub trait EmitFixed2048Cpi<'info> {
    fn emit_fixed_2048<'a>(
        &self,
        accounts: EmitFixed2048CpiAccounts<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        self.emit_fixed_2048_with_remaining_accounts(EmitFixed2048CpiCall {
        accounts,
        signer_seeds: &[],
        remaining_accounts: &[],
    })
    ...
    fn emit_fixed_2048_with_remaining_accounts<'a>(
        &self,
        call: EmitFixed2048CpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()>;

}

#[cfg(feature = "cpi")]
impl<'info> EmitFixed2048Cpi<'info> for crate::sdk_core_cpi::Program<crate::EventsZc> {
    fn emit_fixed_2048_with_remaining_accounts<'a>(
        &self,
        call: EmitFixed2048CpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        let EmitFixed2048CpiCall { accounts, signer_seeds, remaining_accounts } = call;
        let mut ix_data = crate::sdk_core_cpi::Vec::new();
        ix_data.extend_from_slice(&[180, 35, 94, 49, 242, 85, 196, 100]);

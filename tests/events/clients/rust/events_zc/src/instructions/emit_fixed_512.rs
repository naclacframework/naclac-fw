...
#[cfg(feature = "cpi")]
pub struct EmitFixed512CpiAccounts<'a> {
    /// SAFETY: only its address is checked; its data is never read.
    pub program: crate::sdk_core_cpi::CpiHandle<'a>,
}

#[cfg(feature = "cpi")]
pub struct EmitFixed512CpiCall<'a> {
    pub accounts: EmitFixed512CpiAccounts<'a>,
    pub signer_seeds: &'a [&'a [&'a [u8]]],
    pub remaining_accounts: &'a [(crate::sdk_core_cpi::AccountInfo, bool, bool)],
}

#[cfg(feature = "cpi")]
pub trait EmitFixed512Cpi<'info> {
    fn emit_fixed_512<'a>(
        &self,
        accounts: EmitFixed512CpiAccounts<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        self.emit_fixed_512_with_remaining_accounts(EmitFixed512CpiCall {
        accounts,
        signer_seeds: &[],
        remaining_accounts: &[],
    })
    ...
    fn emit_fixed_512_with_remaining_accounts<'a>(
        &self,
        call: EmitFixed512CpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()>;

}

#[cfg(feature = "cpi")]
impl<'info> EmitFixed512Cpi<'info> for crate::sdk_core_cpi::Program<crate::EventsZc> {
    fn emit_fixed_512_with_remaining_accounts<'a>(
        &self,
        call: EmitFixed512CpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        let EmitFixed512CpiCall { accounts, signer_seeds, remaining_accounts } = call;
        let mut ix_data = crate::sdk_core_cpi::Vec::new();
        ix_data.extend_from_slice(&[96, 239, 41, 3, 76, 227, 248, 35]);

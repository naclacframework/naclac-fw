...
pub struct EmitViaSelfCpiFixed128Accounts {
...
#[cfg(feature = "offchain")]
/// See `emit_via_self_cpi_fixed_8` — same purpose, 128-byte fixed payload.
pub fn build_emit_via_self_cpi_fixed_128<'a>(
    provider: &'a naclac_client::NaclacProvider,
    program_id: naclac_client::Address,
    accounts: EmitViaSelfCpiFixed128Accounts,
) -> naclac_client::InstructionBuilder<'a> {
    let ix_data = crate::sdk_core_offchain::vec![129, 232, 188, 192, 5, 157, 66, 73];
...
#[cfg(feature = "cpi")]
/// See `emit_via_self_cpi_fixed_8` — same purpose, 128-byte fixed payload.
pub struct EmitViaSelfCpiFixed128CpiAccounts<'a> {
    /// SAFETY: only its address is checked; its data is never read.
    pub program: crate::sdk_core_cpi::CpiHandle<'a>,
}

#[cfg(feature = "cpi")]
pub struct EmitViaSelfCpiFixed128CpiCall<'a> {
    pub accounts: EmitViaSelfCpiFixed128CpiAccounts<'a>,
    pub signer_seeds: &'a [&'a [&'a [u8]]],
    pub remaining_accounts: &'a [(crate::sdk_core_cpi::AccountInfo, bool, bool)],
}

#[cfg(feature = "cpi")]
pub trait EmitViaSelfCpiFixed128Cpi<'info> {
    /// See `emit_via_self_cpi_fixed_8` — same purpose, 128-byte fixed payload.
    fn emit_via_self_cpi_fixed_128<'a>(
        &self,
        accounts: EmitViaSelfCpiFixed128CpiAccounts<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        self.emit_via_self_cpi_fixed_128_with_remaining_accounts(EmitViaSelfCpiFixed128CpiCall {
        accounts,
        signer_seeds: &[],
        remaining_accounts: &[],
    })
    ...
    fn emit_via_self_cpi_fixed_128_signed<'a>(
        &self,
        accounts: EmitViaSelfCpiFixed128CpiAccounts<'a>,
        signer_seeds: &[&[&[u8]]]
    ) -> crate::sdk_core_cpi::Result<()> {
    ...
    fn emit_via_self_cpi_fixed_128_with_remaining_accounts<'a>(
        &self,
        call: EmitViaSelfCpiFixed128CpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()>;

}

#[cfg(feature = "cpi")]
impl<'info> EmitViaSelfCpiFixed128Cpi<'info> for crate::sdk_core_cpi::Program<crate::EventsZc> {
    fn emit_via_self_cpi_fixed_128_with_remaining_accounts<'a>(
        &self,
        call: EmitViaSelfCpiFixed128CpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        let EmitViaSelfCpiFixed128CpiCall { accounts, signer_seeds, remaining_accounts } = call;
        let mut ix_data = crate::sdk_core_cpi::Vec::new();
        ix_data.extend_from_slice(&[129, 232, 188, 192, 5, 157, 66, 73]);

...
#[cfg(feature = "cpi")]
pub struct EmitFixed128CpiAccounts<'a> {
    /// SAFETY: only its address is checked; its data is never read.
    pub program: crate::sdk_core_cpi::CpiHandle<'a>,
}

#[cfg(feature = "cpi")]
pub struct EmitFixed128CpiCall<'a> {
    pub accounts: EmitFixed128CpiAccounts<'a>,
    pub signer_seeds: &'a [&'a [&'a [u8]]],
    pub remaining_accounts: &'a [(crate::sdk_core_cpi::AccountInfo, bool, bool)],
}

#[cfg(feature = "cpi")]
pub trait EmitFixed128Cpi<'info> {
    fn emit_fixed_128<'a>(
        &self,
        accounts: EmitFixed128CpiAccounts<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        self.emit_fixed_128_with_remaining_accounts(EmitFixed128CpiCall {
        accounts,
        signer_seeds: &[],
        remaining_accounts: &[],
    })
    ...
    fn emit_fixed_128_with_remaining_accounts<'a>(
        &self,
        call: EmitFixed128CpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()>;

}

#[cfg(feature = "cpi")]
impl<'info> EmitFixed128Cpi<'info> for crate::sdk_core_cpi::Program<crate::EventsZc> {
    fn emit_fixed_128_with_remaining_accounts<'a>(
        &self,
        call: EmitFixed128CpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        let EmitFixed128CpiCall { accounts, signer_seeds, remaining_accounts } = call;
        let mut ix_data = crate::sdk_core_cpi::Vec::new();
        ix_data.extend_from_slice(&[23, 253, 190, 60, 160, 216, 143, 40]);

...
#[cfg(feature = "cpi")]
pub struct NoCpiBaselineCpiAccounts<'a> {
    /// SAFETY: only its address is checked; its data is never read.
    pub program: crate::sdk_core_cpi::CpiHandle<'a>,
}

#[cfg(feature = "cpi")]
pub struct NoCpiBaselineCpiCall<'a> {
    pub accounts: NoCpiBaselineCpiAccounts<'a>,
    pub signer_seeds: &'a [&'a [&'a [u8]]],
    pub remaining_accounts: &'a [(crate::sdk_core_cpi::AccountInfo, bool, bool)],
}

#[cfg(feature = "cpi")]
pub trait NoCpiBaselineCpi<'info> {
    fn no_cpi_baseline<'a>(
        &self,
        accounts: NoCpiBaselineCpiAccounts<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        self.no_cpi_baseline_with_remaining_accounts(NoCpiBaselineCpiCall {
        accounts,
        signer_seeds: &[],
        remaining_accounts: &[],
    })
    ...
    fn no_cpi_baseline_with_remaining_accounts<'a>(
        &self,
        call: NoCpiBaselineCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()>;

}

#[cfg(feature = "cpi")]
impl<'info> NoCpiBaselineCpi<'info> for crate::sdk_core_cpi::Program<crate::EventsBorsh> {
    fn no_cpi_baseline_with_remaining_accounts<'a>(
        &self,
        call: NoCpiBaselineCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        let NoCpiBaselineCpiCall { accounts, signer_seeds, remaining_accounts } = call;
        let mut ix_data = crate::sdk_core_cpi::Vec::new();
        ix_data.extend_from_slice(&[4, 133, 150, 205, 53, 226, 164, 73]);

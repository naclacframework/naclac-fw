...
pub struct EmitViaSolLogDataBaselineAccounts {
    /// SAFETY: only its address is checked; its data is never read.
    pub program: naclac_client::Address,
}

#[cfg(feature = "offchain")]
pub fn build_emit_via_sol_log_data_baseline<'a>(
    provider: &'a naclac_client::NaclacProvider,
    program_id: naclac_client::Address,
    accounts: EmitViaSolLogDataBaselineAccounts,
) -> naclac_client::InstructionBuilder<'a> {
    let ix_data = crate::sdk_core_offchain::vec![119, 135, 121, 219, 213, 134, 102, 56];
...
#[cfg(feature = "cpi")]
pub struct EmitViaSolLogDataBaselineCpiAccounts<'a> {
    /// SAFETY: only its address is checked; its data is never read.
    pub program: crate::sdk_core_cpi::CpiHandle<'a>,
}

#[cfg(feature = "cpi")]
pub struct EmitViaSolLogDataBaselineCpiCall<'a> {
    pub accounts: EmitViaSolLogDataBaselineCpiAccounts<'a>,
    pub signer_seeds: &'a [&'a [&'a [u8]]],
    pub remaining_accounts: &'a [(crate::sdk_core_cpi::AccountInfo, bool, bool)],
}

#[cfg(feature = "cpi")]
pub trait EmitViaSolLogDataBaselineCpi<'info> {
    fn emit_via_sol_log_data_baseline<'a>(
        &self,
        accounts: EmitViaSolLogDataBaselineCpiAccounts<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        self.emit_via_sol_log_data_baseline_with_remaining_accounts(EmitViaSolLogDataBaselineCpiCall {
        accounts,
        signer_seeds: &[],
        remaining_accounts: &[],
    })
    ...
    fn emit_via_sol_log_data_baseline_signed<'a>(
        &self,
        accounts: EmitViaSolLogDataBaselineCpiAccounts<'a>,
        signer_seeds: &[&[&[u8]]]
    ) -> crate::sdk_core_cpi::Result<()> {
    ...
    fn emit_via_sol_log_data_baseline_with_remaining_accounts<'a>(
        &self,
        call: EmitViaSolLogDataBaselineCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()>;

}

#[cfg(feature = "cpi")]
impl<'info> EmitViaSolLogDataBaselineCpi<'info> for crate::sdk_core_cpi::Program<crate::EventsBorsh> {
    fn emit_via_sol_log_data_baseline_with_remaining_accounts<'a>(
        &self,
        call: EmitViaSolLogDataBaselineCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        let EmitViaSolLogDataBaselineCpiCall { accounts, signer_seeds, remaining_accounts } = call;
        let mut ix_data = crate::sdk_core_cpi::Vec::new();
        ix_data.extend_from_slice(&[119, 135, 121, 219, 213, 134, 102, 56]);

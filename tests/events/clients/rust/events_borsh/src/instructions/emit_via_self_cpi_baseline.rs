...
pub struct EmitViaSelfCpiBaselineAccounts {
...
/// zero accounts, isolating the fixed CU cost of a self-CPI from any
/// per-byte payload cost.
pub fn build_emit_via_self_cpi_baseline<'a>(
    provider: &'a naclac_client::NaclacProvider,
    program_id: naclac_client::Address,
    accounts: EmitViaSelfCpiBaselineAccounts,
) -> naclac_client::InstructionBuilder<'a> {
    let ix_data = crate::sdk_core_offchain::vec![140, 85, 165, 90, 139, 159, 143, 156];
...
/// zero accounts, isolating the fixed CU cost of a self-CPI from any
/// per-byte payload cost.
pub struct EmitViaSelfCpiBaselineCpiAccounts<'a> {
    /// The invoking program's own account. A self-CPI's target program_id is
    /// resolved from the current instruction's own account list, not from
    /// `invoke`'s arguments — so this program must appear here, the same
    /// reason Anchor's `emit_cpi!` requires a `Program<'info, Self>` account.
...
#[cfg(feature = "cpi")]
pub struct EmitViaSelfCpiBaselineCpiCall<'a> {
    pub accounts: EmitViaSelfCpiBaselineCpiAccounts<'a>,
    pub signer_seeds: &'a [&'a [&'a [u8]]],
    pub remaining_accounts: &'a [(crate::sdk_core_cpi::AccountInfo, bool, bool)],
}

#[cfg(feature = "cpi")]
pub trait EmitViaSelfCpiBaselineCpi<'info> {
    /// Pure self-CPI baseline: invokes `log_event` with zero-length payload and
    /// zero accounts, isolating the fixed CU cost of a self-CPI from any
    /// per-byte payload cost.
    fn emit_via_self_cpi_baseline<'a>(
        &self,
        accounts: EmitViaSelfCpiBaselineCpiAccounts<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        self.emit_via_self_cpi_baseline_with_remaining_accounts(EmitViaSelfCpiBaselineCpiCall {
        accounts,
        signer_seeds: &[],
        remaining_accounts: &[],
    })
    ...
    fn emit_via_self_cpi_baseline_signed<'a>(
        &self,
        accounts: EmitViaSelfCpiBaselineCpiAccounts<'a>,
        signer_seeds: &[&[&[u8]]]
    ) -> crate::sdk_core_cpi::Result<()> {
    ...
    fn emit_via_self_cpi_baseline_with_remaining_accounts<'a>(
        &self,
        call: EmitViaSelfCpiBaselineCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()>;

}

#[cfg(feature = "cpi")]
impl<'info> EmitViaSelfCpiBaselineCpi<'info> for crate::sdk_core_cpi::Program<crate::EventsBorsh> {
    fn emit_via_self_cpi_baseline_with_remaining_accounts<'a>(
        &self,
        call: EmitViaSelfCpiBaselineCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        let EmitViaSelfCpiBaselineCpiCall { accounts, signer_seeds, remaining_accounts } = call;
        let mut ix_data = crate::sdk_core_cpi::Vec::new();
        ix_data.extend_from_slice(&[140, 85, 165, 90, 139, 159, 143, 156]);

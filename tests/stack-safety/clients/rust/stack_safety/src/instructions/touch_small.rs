...
pub struct TouchSmallAccounts {
    pub small: naclac_client::Address,
}

#[cfg(feature = "offchain")]
pub fn build_touch_small<'a>(
    provider: &'a naclac_client::NaclacProvider,
    program_id: naclac_client::Address,
    accounts: TouchSmallAccounts,
) -> naclac_client::InstructionBuilder<'a> {
    let ix_data = crate::sdk_core_offchain::vec![245, 154, 103, 173, 4, 44, 30, 49];
    naclac_client::InstructionBuilder::new(provider, program_id, ix_data)
        .account(naclac_client::AccountMeta { address: accounts.small, is_signer: false, is_writable: true }, "small")
}

#[cfg(feature = "cpi")]
pub struct TouchSmallCpiAccounts<'a> {
    pub small: crate::sdk_core_cpi::CpiHandleMut<'a>,
}

#[cfg(feature = "cpi")]
pub struct TouchSmallCpiCall<'a> {
    pub accounts: TouchSmallCpiAccounts<'a>,
    pub signer_seeds: &'a [&'a [&'a [u8]]],
    pub remaining_accounts: &'a [(crate::sdk_core_cpi::AccountInfo, bool, bool)],
}

#[cfg(feature = "cpi")]
pub trait TouchSmallCpi<'info> {
    fn touch_small<'a>(
        &self,
        accounts: TouchSmallCpiAccounts<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        self.touch_small_with_remaining_accounts(TouchSmallCpiCall {
        accounts,
        signer_seeds: &[],
        remaining_accounts: &[],
    })
    ...
    fn touch_small_with_remaining_accounts<'a>(
        &self,
        call: TouchSmallCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()>;

}

#[cfg(feature = "cpi")]
impl<'info> TouchSmallCpi<'info> for crate::sdk_core_cpi::Program<crate::StackSafety> {
    fn touch_small_with_remaining_accounts<'a>(
        &self,
        call: TouchSmallCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        let TouchSmallCpiCall { accounts, signer_seeds, remaining_accounts } = call;
        let mut ix_data = crate::sdk_core_cpi::Vec::new();
        ix_data.extend_from_slice(&[245, 154, 103, 173, 4, 44, 30, 49]);
        #[cfg(feature = "pinocchio")]
        {
            let mut metas_arr = [const { core::mem::MaybeUninit::<crate::sdk_core_cpi::pinocchio::instruction::InstructionAccount>::uninit() }; 32];
            let mut idx = 0;
            metas_arr[idx].write(crate::sdk_core_cpi::pinocchio::instruction::InstructionAccount::writable(accounts.small.info.view.address()));
            idx += 1;
            ...
            let mut h_idx = 0;
            handles_arr[h_idx].write(crate::sdk_core_cpi::CpiHandle::from(accounts.small));
            h_idx += 1;
        ...
        #[cfg(not(feature = "pinocchio"))]
        {
            ...
            let mut account_metas = crate::sdk_core_cpi::vec![
                ::naclac_lang::solana_program::instruction::AccountMeta::new(accounts.small.address(), false),
            ];
            for (acc, is_writable, is_signer) in remaining_accounts {
                let meta = if *is_writable {

...
pub struct TouchBigAccounts {
    pub big: naclac_client::Address,
}

#[cfg(feature = "offchain")]
pub fn build_touch_big<'a>(
    provider: &'a naclac_client::NaclacProvider,
    program_id: naclac_client::Address,
    accounts: TouchBigAccounts,
) -> naclac_client::InstructionBuilder<'a> {
    let ix_data = crate::sdk_core_offchain::vec![66, 178, 192, 160, 63, 148, 8, 37];
    naclac_client::InstructionBuilder::new(provider, program_id, ix_data)
        .account(naclac_client::AccountMeta { address: accounts.big, is_signer: false, is_writable: true }, "big")
}

#[cfg(feature = "cpi")]
pub struct TouchBigCpiAccounts<'a> {
    pub big: crate::sdk_core_cpi::CpiHandleMut<'a>,
}

#[cfg(feature = "cpi")]
pub struct TouchBigCpiCall<'a> {
    pub accounts: TouchBigCpiAccounts<'a>,
    pub signer_seeds: &'a [&'a [&'a [u8]]],
    pub remaining_accounts: &'a [(crate::sdk_core_cpi::AccountInfo, bool, bool)],
}

#[cfg(feature = "cpi")]
pub trait TouchBigCpi<'info> {
    fn touch_big<'a>(
        &self,
        accounts: TouchBigCpiAccounts<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        self.touch_big_with_remaining_accounts(TouchBigCpiCall {
        accounts,
        signer_seeds: &[],
        remaining_accounts: &[],
    })
    ...
    fn touch_big_with_remaining_accounts<'a>(
        &self,
        call: TouchBigCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()>;

}

#[cfg(feature = "cpi")]
impl<'info> TouchBigCpi<'info> for crate::sdk_core_cpi::Program<crate::StackSafety> {
    fn touch_big_with_remaining_accounts<'a>(
        &self,
        call: TouchBigCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        let TouchBigCpiCall { accounts, signer_seeds, remaining_accounts } = call;
        let mut ix_data = crate::sdk_core_cpi::Vec::new();
        ix_data.extend_from_slice(&[66, 178, 192, 160, 63, 148, 8, 37]);
        #[cfg(feature = "pinocchio")]
        {
            let mut metas_arr = [const { core::mem::MaybeUninit::<crate::sdk_core_cpi::pinocchio::instruction::InstructionAccount>::uninit() }; 32];
            let mut idx = 0;
            metas_arr[idx].write(crate::sdk_core_cpi::pinocchio::instruction::InstructionAccount::writable(accounts.big.info.view.address()));
            idx += 1;
            ...
            let mut h_idx = 0;
            handles_arr[h_idx].write(crate::sdk_core_cpi::CpiHandle::from(accounts.big));
            h_idx += 1;
        ...
        #[cfg(not(feature = "pinocchio"))]
        {
            ...
            let mut account_metas = crate::sdk_core_cpi::vec![
                ::naclac_lang::solana_program::instruction::AccountMeta::new(accounts.big.address(), false),
            ];
            for (acc, is_writable, is_signer) in remaining_accounts {
                let meta = if *is_writable {

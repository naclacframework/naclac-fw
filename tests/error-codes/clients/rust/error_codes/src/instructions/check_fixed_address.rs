...
pub struct CheckFixedAddressAccounts {
...
#[cfg(feature = "offchain")]
pub fn build_check_fixed_address<'a>(
    provider: &'a naclac_client::NaclacProvider,
    program_id: naclac_client::Address,
    accounts: CheckFixedAddressAccounts,
) -> naclac_client::InstructionBuilder<'a> {
    let ix_data = crate::sdk_core_offchain::vec![171, 46, 251, 206, 177, 201, 101, 156];
...
#[cfg(feature = "cpi")]
pub struct CheckFixedAddressCpiAccounts<'a> {
    /// SAFETY: only this account's own address is checked against the
    /// constant below — its data is never read or deserialized.
...
#[cfg(feature = "cpi")]
pub struct CheckFixedAddressCpiCall<'a> {
    pub accounts: CheckFixedAddressCpiAccounts<'a>,
    pub signer_seeds: &'a [&'a [&'a [u8]]],
    pub remaining_accounts: &'a [(crate::sdk_core_cpi::AccountInfo, bool, bool)],
}

#[cfg(feature = "cpi")]
pub trait CheckFixedAddressCpi<'info> {
    fn check_fixed_address<'a>(
        &self,
        accounts: CheckFixedAddressCpiAccounts<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        self.check_fixed_address_with_remaining_accounts(CheckFixedAddressCpiCall {
        accounts,
        signer_seeds: &[],
        remaining_accounts: &[],
    })
    ...
    fn check_fixed_address_signed<'a>(
        &self,
        accounts: CheckFixedAddressCpiAccounts<'a>,
        signer_seeds: &[&[&[u8]]]
    ) -> crate::sdk_core_cpi::Result<()> {
    ...
    fn check_fixed_address_with_remaining_accounts<'a>(
        &self,
        call: CheckFixedAddressCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()>;

}

#[cfg(feature = "cpi")]
impl<'info> CheckFixedAddressCpi<'info> for crate::sdk_core_cpi::Program<crate::ErrorCodes> {
    fn check_fixed_address_with_remaining_accounts<'a>(
        &self,
        call: CheckFixedAddressCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        let CheckFixedAddressCpiCall { accounts, signer_seeds, remaining_accounts } = call;
        let mut ix_data = crate::sdk_core_cpi::Vec::new();
        ix_data.extend_from_slice(&[171, 46, 251, 206, 177, 201, 101, 156]);

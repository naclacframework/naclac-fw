...
pub struct CheckAuthorityAccounts {
    ...
    pub required: naclac_client::Address,
}

#[cfg(feature = "offchain")]
pub fn build_check_authority<'a>(
    provider: &'a naclac_client::NaclacProvider,
    program_id: naclac_client::Address,
    accounts: CheckAuthorityAccounts,
) -> naclac_client::InstructionBuilder<'a> {
    let ix_data = crate::sdk_core_offchain::vec![150, 20, 117, 137, 30, 218, 200, 87];
    naclac_client::InstructionBuilder::new(provider, program_id, ix_data)
        ...
        .account(naclac_client::AccountMeta { address: accounts.required, is_signer: false, is_writable: false }, "required")
}

#[cfg(feature = "cpi")]
pub struct CheckAuthorityCpiAccounts<'a> {
    pub payer: crate::sdk_core_cpi::CpiHandle<'a>,
    /// SAFETY: only this account's own address is compared against
    /// `payer`'s — its data is never read or deserialized.
    pub required: crate::sdk_core_cpi::CpiHandle<'a>,
}

#[cfg(feature = "cpi")]
pub struct CheckAuthorityCpiCall<'a> {
    pub accounts: CheckAuthorityCpiAccounts<'a>,
    pub signer_seeds: &'a [&'a [&'a [u8]]],
    pub remaining_accounts: &'a [(crate::sdk_core_cpi::AccountInfo, bool, bool)],
}

#[cfg(feature = "cpi")]
pub trait CheckAuthorityCpi<'info> {
    fn check_authority<'a>(
        &self,
        accounts: CheckAuthorityCpiAccounts<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        self.check_authority_with_remaining_accounts(CheckAuthorityCpiCall {
        accounts,
        signer_seeds: &[],
        remaining_accounts: &[],
    })
    ...
    fn check_authority_with_remaining_accounts<'a>(
        &self,
        call: CheckAuthorityCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()>;

}

#[cfg(feature = "cpi")]
impl<'info> CheckAuthorityCpi<'info> for crate::sdk_core_cpi::Program<crate::ErrorCodes> {
    fn check_authority_with_remaining_accounts<'a>(
        &self,
        call: CheckAuthorityCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        let CheckAuthorityCpiCall { accounts, signer_seeds, remaining_accounts } = call;
        let mut ix_data = crate::sdk_core_cpi::Vec::new();
        ix_data.extend_from_slice(&[150, 20, 117, 137, 30, 218, 200, 87]);
        #[cfg(feature = "pinocchio")]
        {
            ...
            idx += 1;
            metas_arr[idx].write(crate::sdk_core_cpi::pinocchio::instruction::InstructionAccount::readonly(accounts.required.info.view.address()));
            idx += 1;
            ...
            h_idx += 1;
            handles_arr[h_idx].write(accounts.required);
            h_idx += 1;
        ...
        #[cfg(not(feature = "pinocchio"))]
        {
            ...
            let mut account_metas = crate::sdk_core_cpi::vec![
                ...
                ::naclac_lang::solana_program::instruction::AccountMeta::new_readonly(accounts.required.address(), false),
            ];
            for (acc, is_writable, is_signer) in remaining_accounts {
                let meta = if *is_writable {

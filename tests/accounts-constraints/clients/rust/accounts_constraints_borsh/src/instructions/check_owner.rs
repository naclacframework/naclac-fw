...
#[cfg(feature = "offchain")]
pub fn build_check_owner<'a>(
    provider: &'a naclac_client::NaclacProvider,
    program_id: naclac_client::Address,
    accounts: CheckOwnerAccounts,
) -> naclac_client::InstructionBuilder<'a> {
    let ix_data = crate::sdk_core_offchain::vec![215, 161, 49, 182, 161, 8, 67, 193];
...
#[cfg(feature = "cpi")]
pub struct CheckOwnerCpiAccounts<'a> {
    /// SAFETY: only the account's owner field is inspected via the `owner`
    /// constraint below; its data is never read or deserialized.
...
#[cfg(feature = "cpi")]
pub struct CheckOwnerCpiCall<'a> {
    pub accounts: CheckOwnerCpiAccounts<'a>,
    pub signer_seeds: &'a [&'a [&'a [u8]]],
    pub remaining_accounts: &'a [(crate::sdk_core_cpi::AccountInfo, bool, bool)],
}

#[cfg(feature = "cpi")]
pub trait CheckOwnerCpi<'info> {
    fn check_owner<'a>(
        &self,
        accounts: CheckOwnerCpiAccounts<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        self.check_owner_with_remaining_accounts(CheckOwnerCpiCall {
        accounts,
        signer_seeds: &[],
        remaining_accounts: &[],
    })
    ...
    fn check_owner_with_remaining_accounts<'a>(
        &self,
        call: CheckOwnerCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()>;

}

#[cfg(feature = "cpi")]
impl<'info> CheckOwnerCpi<'info> for crate::sdk_core_cpi::Program<crate::AccountsConstraintsBorsh> {
    fn check_owner_with_remaining_accounts<'a>(
        &self,
        call: CheckOwnerCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        let CheckOwnerCpiCall { accounts, signer_seeds, remaining_accounts } = call;
        let mut ix_data = crate::sdk_core_cpi::Vec::new();
        ix_data.extend_from_slice(&[215, 161, 49, 182, 161, 8, 67, 193]);

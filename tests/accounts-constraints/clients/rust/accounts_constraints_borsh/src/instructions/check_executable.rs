...
pub struct CheckExecutableAccounts {
...
#[cfg(feature = "offchain")]
pub fn build_check_executable<'a>(
    provider: &'a naclac_client::NaclacProvider,
    program_id: naclac_client::Address,
    accounts: CheckExecutableAccounts,
) -> naclac_client::InstructionBuilder<'a> {
    let ix_data = crate::sdk_core_offchain::vec![27, 192, 107, 127, 85, 227, 212, 55];
...
#[cfg(feature = "cpi")]
pub struct CheckExecutableCpiAccounts<'a> {
    /// SAFETY: only the account's `executable` flag is inspected via the
    /// `executable` constraint below; its data is never read or deserialized.
...
#[cfg(feature = "cpi")]
pub struct CheckExecutableCpiCall<'a> {
    pub accounts: CheckExecutableCpiAccounts<'a>,
    pub signer_seeds: &'a [&'a [&'a [u8]]],
    pub remaining_accounts: &'a [(crate::sdk_core_cpi::AccountInfo, bool, bool)],
}

#[cfg(feature = "cpi")]
pub trait CheckExecutableCpi<'info> {
    fn check_executable<'a>(
        &self,
        accounts: CheckExecutableCpiAccounts<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        self.check_executable_with_remaining_accounts(CheckExecutableCpiCall {
        accounts,
        signer_seeds: &[],
        remaining_accounts: &[],
    })
    ...
    fn check_executable_with_remaining_accounts<'a>(
        &self,
        call: CheckExecutableCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()>;

}

#[cfg(feature = "cpi")]
impl<'info> CheckExecutableCpi<'info> for crate::sdk_core_cpi::Program<crate::AccountsConstraintsBorsh> {
    fn check_executable_with_remaining_accounts<'a>(
        &self,
        call: CheckExecutableCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        let CheckExecutableCpiCall { accounts, signer_seeds, remaining_accounts } = call;
        let mut ix_data = crate::sdk_core_cpi::Vec::new();
        ix_data.extend_from_slice(&[27, 192, 107, 127, 85, 227, 212, 55]);

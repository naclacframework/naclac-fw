...
pub struct CheckOwnerRelationalAccounts {
    ...
    /// never deserialized.
    pub expected_owner: naclac_client::Address,
}

#[cfg(feature = "offchain")]
pub fn build_check_owner_relational<'a>(
    provider: &'a naclac_client::NaclacProvider,
    program_id: naclac_client::Address,
    accounts: CheckOwnerRelationalAccounts,
) -> naclac_client::InstructionBuilder<'a> {
    let ix_data = crate::sdk_core_offchain::vec![179, 162, 242, 171, 88, 217, 216, 174];
    naclac_client::InstructionBuilder::new(provider, program_id, ix_data)
        ...
        .account(naclac_client::AccountMeta { address: accounts.expected_owner, is_signer: false, is_writable: false }, "expected_owner")
}

#[cfg(feature = "cpi")]
pub struct CheckOwnerRelationalCpiAccounts<'a> {
    /// SAFETY: only the account's owner field is inspected via the `owner`
    /// constraint below; its data is never read or deserialized.
    pub target: crate::sdk_core_cpi::CpiHandle<'a>,
    /// SAFETY: only its own address is read, as the expected owning program;
    /// never deserialized.
    pub expected_owner: crate::sdk_core_cpi::CpiHandle<'a>,
}

#[cfg(feature = "cpi")]
pub struct CheckOwnerRelationalCpiCall<'a> {
    pub accounts: CheckOwnerRelationalCpiAccounts<'a>,
    pub signer_seeds: &'a [&'a [&'a [u8]]],
    pub remaining_accounts: &'a [(crate::sdk_core_cpi::AccountInfo, bool, bool)],
}

#[cfg(feature = "cpi")]
pub trait CheckOwnerRelationalCpi<'info> {
    fn check_owner_relational<'a>(
        &self,
        accounts: CheckOwnerRelationalCpiAccounts<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        self.check_owner_relational_with_remaining_accounts(CheckOwnerRelationalCpiCall {
        accounts,
        signer_seeds: &[],
        remaining_accounts: &[],
    })
    ...
    fn check_owner_relational_signed<'a>(
        &self,
        accounts: CheckOwnerRelationalCpiAccounts<'a>,
        signer_seeds: &[&[&[u8]]]
    ) -> crate::sdk_core_cpi::Result<()> {
    ...
    fn check_owner_relational_with_remaining_accounts<'a>(
        &self,
        call: CheckOwnerRelationalCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()>;

}

#[cfg(feature = "cpi")]
impl<'info> CheckOwnerRelationalCpi<'info> for crate::sdk_core_cpi::Program<crate::AccountsConstraintsBorsh> {
    fn check_owner_relational_with_remaining_accounts<'a>(
        &self,
        call: CheckOwnerRelationalCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        let CheckOwnerRelationalCpiCall { accounts, signer_seeds, remaining_accounts } = call;
        let mut ix_data = crate::sdk_core_cpi::Vec::new();
        ix_data.extend_from_slice(&[179, 162, 242, 171, 88, 217, 216, 174]);
        #[cfg(feature = "pinocchio")]
        {
            ...
            idx += 1;
            metas_arr[idx].write(crate::sdk_core_cpi::pinocchio::instruction::InstructionAccount::readonly(accounts.expected_owner.info.view.address()));
            idx += 1;
            ...
            h_idx += 1;
            handles_arr[h_idx].write(accounts.expected_owner);
            h_idx += 1;
        ...
        #[cfg(not(feature = "pinocchio"))]
        {
            ...
            let mut account_metas = crate::sdk_core_cpi::vec![
                ...
                ::naclac_lang::solana_program::instruction::AccountMeta::new_readonly(accounts.expected_owner.address(), false),
            ];
            for (acc, is_writable, is_signer) in remaining_accounts {
                let meta = if *is_writable {

use naclac_client::*;
use accounts_constraints_zc_client::{
    ...
    instructions::{
        ...
        build_init_if_needed_ledger, build_init_seeded, build_init_vault, build_related_vault,
        ...
        InitSeededAccounts, InitVaultAccounts, RelatedVaultAccounts,
...
fn load_program(provider: &NaclacProvider) {
    ...
    so_path.push("target/deploy/accounts_constraints_zc.so");

    provider
        .add_program(&PROGRAM_ID, so_path.to_str().unwrap())
        .expect("Failed to load accounts_constraints_zc program binary");
}

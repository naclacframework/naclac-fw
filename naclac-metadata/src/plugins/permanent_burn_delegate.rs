// ===========================================================================
// plugins/permanent_burn_delegate.rs — the `PermanentBurnDelegate` plugin
// ===========================================================================

//! `PermanentBurnDelegate` — like `BurnDelegate`, but attached at creation
//! time and permanent (can't be removed) for the life of the asset. Real
//! layout verified against `mpl-core`'s
//! `generated::types::PermanentBurnDelegate`: an empty struct (Borsh
...
/// Attaches `PermanentBurnDelegate` to an `Asset` via `add_plugin`.
pub fn attach_permanent_burn_delegate_signed(
    program: CpiHandle<'_>,
    ...
    {
        let data = [2u8, plugin_type::PERMANENT_BURN_DELEGATE, 0u8];
    ...
    #[cfg(feature = "pinocchio")]
    {
        ...
        data.push(plugin_type::PERMANENT_BURN_DELEGATE);
        data.push(0u8); // init_authority: None
        add_asset_plugin_signed_pinocchio(program, accounts, data.as_slice(), signer_seeds)
    }
}

/// Attaches `PermanentBurnDelegate` to a `Collection` via
/// `add_collection_plugin`.
pub fn attach_collection_permanent_burn_delegate_signed(
    program: CpiHandle<'_>,
    ...
    #[cfg(not(feature = "pinocchio"))]
    {
        let data = [3u8, plugin_type::PERMANENT_BURN_DELEGATE, 0u8];
...
/// `true` if an `Asset` has a `PermanentBurnDelegate` plugin attached.
/// Callable identically on both backends.
#[cfg(not(feature = "pinocchio"))]
pub fn asset_has_permanent_burn_delegate(info: &AccountInfo) -> Result<bool> {
    ...
    Ok(find_plugin_offset(&data, plugin_header_offset, plugin_type::PERMANENT_BURN_DELEGATE)?
        .is_some())
}

/// `true` if a `Collection` has a `PermanentBurnDelegate` plugin attached.
/// Callable identically on both backends.
#[cfg(not(feature = "pinocchio"))]
pub fn collection_has_permanent_burn_delegate(info: &AccountInfo) -> Result<bool> {
    if Owner::program_owner(info) != crate::ID {
        return Err(NaclacError::ConstraintOwner.into());
    }
    let solana_info = unsafe { info.to_lifetime() };
...
pub fn asset_has_permanent_burn_delegate(info: &AccountInfo) -> Result<bool> {
    ...
    Ok(find_plugin_offset(
        ...
        plugin_type::PERMANENT_BURN_DELEGATE,
    )?
    .is_some())
}

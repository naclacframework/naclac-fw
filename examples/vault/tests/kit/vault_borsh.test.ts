...
import { VaultBorshClient } from "../../clients/typescript/src/generated/vault_borsh";

describe("Naclac Vault Test Suite", () => {
  let client: VaultBorshClient;
  ...
  before(async () => {
    ...
    client = new VaultBorshClient("localnet", payer);
    [vaultPda] = await client.getVaultAccountPda({});
    [userAccountPda, userBump] = await client.getUserAccountPda({ user: payer.address });
  });
  ...
  it("1. Setup & Initialize Vault", async () => {
    ...
    try {
      ...
      await client
        .initialize({})
        .accounts({ payer: payer.address, vaultAccount: vaultPda })
        .rpc()
  ...
  it("2. Deposit and Verify Balance", async () => {
    ...
    console.log(`      ✅ Vault Total: ${vaultState.data.totalDeposited}`);
    console.log(`      ✅ User Balance: ${userState.data.balance}`);

    if (Number(userState.data.balance) < amount) {

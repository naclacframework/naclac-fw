//! Follow-up to `probe57.rs` through `probe60.rs`, which DISPROVED all four
//! individual `bonding_curve` reserve fields as the "already migrated"
//! check: `virtual_token_reserves`, `real_token_reserves`,
//! `real_quote_reserves`, `virtual_quote_reserves` each tested alone all
//! let the real program run its full CPI sequence.
//!
//! New hypothesis, not yet tested: `migrate.rs`'s own module comment
//! already notes real `migrate` moves `associated_bonding_curve`'s ACTUAL
//! token balance, not the `bonding_curve.real_token_reserves` counter — so
//! the "anything left to migrate" check may read that separate token
//! account's real balance directly, never a `bonding_curve` struct field at
//! all. Every probe so far left `associated_bonding_curve`'s actual balance
//! nonzero throughout. This probe flips that: `bonding_curve`'s own fields
//! are ALL left genuinely nonzero (as if never migrated), but
//! `associated_bonding_curve`'s real token amount is set to 0.
//!
...
fn main() {
    ...
    // every `bonding_curve` struct field stays genuinely nonzero (as if
    // never migrated), but `associated_bonding_curve`'s ACTUAL token
    // balance (a separate account, not a `bonding_curve` field) is 0. If
    // real `pump.so` still treats this as "already migrated" and no-ops,
    // that isolates the real token-account balance as the exact check, not
    // any `bonding_curve` struct field at all.
    ...
    svm.set_account(
        ...
        Account {
            ...
            data: bonding_curve_account_data(&creator, true, VIRTUAL_TOKEN_RESERVES, VIRTUAL_SOL_RESERVES, REAL_TOKEN_RESERVES, REAL_SOL_RESERVES),
            owner: pump_program,
            executable: false,
            rent_epoch: 0,
        },
    ...
    run_migrate(
        ...
        &user,
        &ix,
        "SINGLE migrate call (associated_bonding_curve real balance=0 only, all bonding_curve fields genuinely nonzero, no prior migration ever ran)",
    );
    ...
    println!("directly isolates associated_bonding_curve's real token balance as the");
    println!("check -- not any bonding_curve struct field at all, since every one of");
    println!("those was left genuinely nonzero here. If it instead proceeded into real");
    println!("CPIs (and likely failed on insufficient token balance), the token-account");
    println!("balance isn't the check either -- back to a compound bonding_curve-field");
    println!("condition or something else entirely.");
}

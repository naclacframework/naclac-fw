//! Probes the real `pump_amm.so`'s `create_pool` to empirically determine
//! its initial-liquidity bootstrap math (LP token supply formula, any
//! minimum-liquidity lock/burn, any fee taken on the initial deposit) rather
//! than assume a textbook Uniswap-v2 `sqrt(x*y)` formula. Needed to
//! faithfully reimplement `create_pool` in our own `pump-amm` (user
//! explicitly chose the full, general reimplementation over a scoped one).
//!
//! Strategy: fund a real signer with base_mint + WSOL, seed a `GlobalConfig`
//! with create-pool enabled, call the real `create_pool`, then inspect the
//! resulting `Pool` account, LP mint supply, and all three token account
//! balances directly.
...
// admin, lp_fee_basis_points, protocol_fee_basis_points, disable_flags,
// protocol_fee_recipients[8], coin_creator_fee_basis_points,
...
// buyback_fee_recipients[8], buyback_basis_points, boost_authority,
// boost_enabled — confirmed field order from pump-public-docs/idl/pump_amm.json.
...
fn main() {
    ...
    match svm.send_transaction(tx) {
        Ok(meta) => {
            ...
            println!("\npool raw data ({} bytes): {:02x?}", pool_account.data.len(), pool_account.data);
            println!("discriminator matches expected: {}", pool_account.data[0..8] == POOL_DISCRIMINATOR);

            let lp_mint_account = svm.get_account(&lp_mint).expect("lp_mint must exist");
            println!("\nlp_mint raw data ({} bytes): {:02x?}", lp_mint_account.data.len(), lp_mint_account.data);
            println!("lp_mint owner = {} (token_2022={})", lp_mint_account.owner, lp_mint_account.owner == token_2022_program);

            let user_pool_ata = svm.get_account(&user_pool_token_account).expect("user_pool_token_account must exist");
            println!("\nuser_pool_token_account raw data ({} bytes): {:02x?}", user_pool_ata.data.len(), user_pool_ata.data);

            let pool_base_ata = svm.get_account(&pool_base_token_account).expect("pool_base_token_account must exist");
            println!("\npool_base_token_account balance bytes: {:02x?}", &pool_base_ata.data[64..72]);

            let pool_quote_ata = svm.get_account(&pool_quote_token_account).expect("pool_quote_token_account must exist");
            println!("pool_quote_token_account balance bytes: {:02x?}", &pool_quote_ata.data[64..72]);
        }
        Err(e) => {

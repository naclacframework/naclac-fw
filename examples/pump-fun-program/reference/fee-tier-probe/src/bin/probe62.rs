//! Follow-up to `probe57.rs` through `probe61.rs`, which DISPROVED five
//! individual candidates: each of `bonding_curve`'s four reserve fields
//! alone (`virtual_token_reserves`, `real_token_reserves`,
//! `real_quote_reserves`, `virtual_quote_reserves`), and
//! `associated_bonding_curve`'s real token balance alone — every one of
//! those, tested in isolation, let the real program run its full CPI
//! sequence (two even proceeded all the way into `pump_amm::create_pool`,
//! failing there on ITS OWN downstream checks, not `migrate`'s).
//!
//! Every real "already migrated" `bonding_curve` fetched from mainnet had
//! ALL FOUR reserve fields at 0 simultaneously — this probe tests the most
//! semantically plausible compound pairing: `real_token_reserves == 0 &&
//! real_quote_reserves == 0` together (the two fields our own `migrate.rs`
//! already zeroes at the end), with `virtual_token_reserves`/
//! `virtual_quote_reserves` left genuinely nonzero. If real `pump.so`
//! still no-ops here, we don't even need new zeroing logic — our existing
//! post-migration state already produces exactly the condition the check
//! needs; we'd only need to add the check itself.
//!
...
fn main() {
    ...
    // `real_token_reserves`/`real_quote_reserves` are BOTH pre-zeroed
    // together, while `virtual_token_reserves`/`virtual_quote_reserves`
    // stay genuinely nonzero, and `associated_bonding_curve`'s real token
    // balance is nonzero too. If real `pump.so` still treats this as
    // "already migrated" and no-ops, that isolates this compound pair as
    // the exact check.
    ...
    svm.set_account(
        ...
        Account {
            ...
            data: bonding_curve_account_data(&creator, true, VIRTUAL_TOKEN_RESERVES, VIRTUAL_SOL_RESERVES, 0, 0),
            owner: pump_program,
            executable: false,
            rent_epoch: 0,
        },
    ...
    run_migrate(
        ...
        &user,
        &ix,
        "SINGLE migrate call (real_token_reserves=0 AND real_quote_reserves=0 together, virtual fields genuinely nonzero, no prior migration ever ran)",
    );
    ...
    println!("directly isolates real_token_reserves==0 && real_quote_reserves==0 together");
    println!("as the real check. If it instead proceeded into real CPIs, this compound");
    println!("pair isn't it either -- next candidate would be all four reserve fields");
    println!("zero together, or something entirely outside bonding_curve's own fields.");
}

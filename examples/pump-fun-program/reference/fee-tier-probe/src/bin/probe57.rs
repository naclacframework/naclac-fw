//! Follow-up to `probe56.rs`, which found that calling real `migrate` twice
//! in a row on the same curve makes the second call succeed as a no-op
//! ("Bonding curve already migrated", zero inner instructions), and that
//! the first call zeroes ALL FOUR reserve fields
//! (`virtual_token_reserves`/`virtual_quote_reserves` too, not just
//! `real_token_reserves`/`real_quote_reserves`, which is all our own
//! `migrate.rs`/`migrate_v2.rs` currently zero). That's strong correlating
//! evidence `virtual_token_reserves == 0` is the actual "already migrated"
//! check, but it's still an inference from two sequential calls, not a
//! direct isolation of which field is being read.
//!
//! This probe isolates it directly: construct a bonding curve fixture with
//! ONLY `virtual_token_reserves` pre-set to 0 — `virtual_quote_reserves`,
//! `real_token_reserves`, `real_quote_reserves` all left genuinely nonzero,
//! a state a real first migration would never actually produce — and call
//! `migrate` on it a single time, with no prior migration ever having run.
//! If real `pump.so` still reports "already migrated" and does nothing,
//! that proves the check reads `virtual_token_reserves` specifically (not
//! some other field, and not something inferred from the two-call
//! sequence's overall correlation).
//!
//! Usage: cargo run --bin probe57
...
fn bonding_curve_account_data(creator: &Pubkey, complete: bool, virtual_token_reserves: u64, real_token_reserves: u64, real_sol_reserves: u64) -> Vec<u8> {
    let mut data = Vec::new();
...
fn main() {
    ...
    // Deliberately NOT what a real first migration would ever produce:
    // `virtual_token_reserves` alone is pre-zeroed, while
    // `virtual_quote_reserves`/`real_token_reserves`/`real_quote_reserves`
    // stay genuinely nonzero. If real `pump.so` still treats this as
    // "already migrated" and no-ops, that isolates `virtual_token_reserves`
    // as the exact field being checked, not an artifact of a real
    // migration's full effect.
    ...
    svm.set_account(
        ...
        Account {
            ...
            data: bonding_curve_account_data(&creator, true, 0, REAL_TOKEN_RESERVES, REAL_SOL_RESERVES),
            owner: pump_program,
            executable: false,
            rent_epoch: 0,
        },
    ...
    svm.set_account(associated_bonding_curve, Account { lamports: 2_039_280, data: token_account_data(&mint, &bonding_curve, REAL_TOKEN_RESERVES), owner: token_program, executable: false, rent_epoch: 0 }).unwrap();
    ...
    run_migrate(
        ...
        &user,
        &ix,
        "SINGLE migrate call (virtual_token_reserves=0 only, real reserves genuinely nonzero, no prior migration ever ran)",
    );

    println!("\nCONCLUSION: if this SUCCEEDED with zero inner instructions and the same");
    println!("\"Bonding curve already migrated\" log line as probe56's second call, that");
    println!("directly isolates virtual_token_reserves==0 as the real check -- not an");
    println!("artifact of a real migration's combined effect, since real_token_reserves/");
    println!("real_quote_reserves/virtual_quote_reserves were all left genuinely nonzero");
    println!("here and pool/lp_mint/etc. never existed to begin with. If it instead");
    println!("proceeded into real CPIs (and likely failed on inconsistent state), the");
    println!("check reads something else, not virtual_token_reserves.");
}

//! Follow-up to `probe57.rs`, which DISPROVED the hypothesis that
//! `virtual_token_reserves == 0` alone triggers real `migrate`'s
//! "already migrated" no-op: with only `virtual_token_reserves` pre-zeroed
//! (everything else genuinely nonzero), the real program ran the ENTIRE
//! CPI sequence normally (`CreatePool`, `MintTo`, `Burn`, `CloseAccount`,
//! all of it) — not the no-op path at all.
//!
//! This probe tests the next candidate: `real_token_reserves == 0` (a field
//! our own `migrate.rs` already zeroes at the end, and which real `migrate`
//! was also observed zeroing in `probe56`). Fixture here: `real_token_reserves`
//! pre-set to 0, `virtual_token_reserves`/`virtual_quote_reserves`/
//! `real_quote_reserves` all left genuinely nonzero — no prior migration
//! ever ran. If real `pump.so` reports "already migrated" and does nothing,
//! that isolates `real_token_reserves` as the real check.
//!
//! Usage: cargo run --bin probe58
...
fn main() {
    ...
    // `real_token_reserves` alone is pre-zeroed, while
    // `virtual_token_reserves`/`virtual_quote_reserves`/`real_quote_reserves`
    // stay genuinely nonzero. If real `pump.so` still treats this as
    // "already migrated" and no-ops, that isolates `real_token_reserves`
    ...
    svm.set_account(
        ...
        Account {
            ...
            data: bonding_curve_account_data(&creator, true, VIRTUAL_TOKEN_RESERVES, 0, REAL_SOL_RESERVES),
            owner: pump_program,
            executable: false,
            rent_epoch: 0,
        },
    ...
    run_migrate(
        ...
        &user,
        &ix,
        "SINGLE migrate call (real_token_reserves=0 only, everything else genuinely nonzero, no prior migration ever ran)",
    );
    ...
    println!("directly isolates real_token_reserves==0 as the real check -- not an");
    println!("artifact of a real migration's combined effect, since virtual_token_reserves/");
    println!("virtual_quote_reserves/real_quote_reserves were all left genuinely nonzero");
    ...
    println!("check reads something else, not real_token_reserves.");
}

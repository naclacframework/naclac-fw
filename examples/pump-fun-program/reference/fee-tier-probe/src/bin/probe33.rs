//! Follow-up to `probe32.rs`, which proved `migrate` does NOT internally fund
//! `pool_authority` from `bonding_curve`/`withdraw_authority`/`user` — with
//! `pool_authority` at 0 lamports the tx failed immediately on the first
//! account-creation rent transfer. But `pump.json`'s `Global` struct has a
//! `pool_migration_fee: u64` field (zeroed in every probe so far) plus two
//! errors that only make sense if it's funded FROM the bonding curve's own
//! reserve: `PoolMigrationFeeShouldBeLessThanFinalRealSolReserves` and
//! `PoolMigrationFeeShouldBeGreaterThanCreatorFeePlusMaxMigrateFees`. This
//! probe tests that theory directly: `pool_migration_fee` set to a nonzero,
//! plausible value, `pool_authority` still starting at 0 lamports. If the tx
//! now succeeds, and `pool_quote_token_account` ends up with
//! `real_sol_reserves - pool_migration_fee` (not the full amount), that
//! confirms `pool_migration_fee` is carved out of the bonding curve's own SOL
//! to pay for `pool_authority`'s rent, not sourced externally.
...
// `migrate` to accept the call. `pool_migration_fee` is the value under test
// here — see file header.
fn global_account_data(withdraw_authority: &Pubkey, pool_migration_fee: u64) -> Vec<u8> {
    ...
    data.extend_from_slice(&pool_migration_fee.to_le_bytes()); // pool_migration_fee
...
fn main() {
    ...
    // Must satisfy `pool_migration_fee < real_sol_reserves` and
    // `pool_migration_fee > creator_fee + MAX_MIGRATE_FEES` (creator_fee = 0
    // here, no accumulated creator fees in this fixture) per the real
    // program's own validation — 15_000_001 matches the value cited in
    // pump-fun's public docs.
    const POOL_MIGRATION_FEE: u64 = 15_000_001;
    let (global, _) = Pubkey::find_program_address(&[b"global"], &pump_program);
    svm.set_account(
        global,
        Account {
            lamports: 10_000_000,
            data: global_account_data(&withdraw_authority.pubkey(), POOL_MIGRATION_FEE),
            owner: pump_program,
            executable: false,
            rent_epoch: 0,
        },
    ...
    match svm.send_transaction(tx) {
        Ok(meta) => {
            ...
            if let Some(upta) = svm.get_account(&user_pool_token_account) {
                let balance = u64::from_le_bytes(upta.data[64..72].try_into().unwrap());
                println!("user_pool_token_account balance = {balance}");
            } else {
                println!("user_pool_token_account does NOT exist post-tx");
            }
            if let Some(pbta) = svm.get_account(&pool_base_token_account) {
                let balance = u64::from_le_bytes(pbta.data[64..72].try_into().unwrap());
                println!("pool_base_token_account balance = {balance} (REAL_TOKEN_RESERVES = {REAL_TOKEN_RESERVES})");
            }
            if let Some(pqta) = svm.get_account(&pool_quote_token_account) {
                let balance = u64::from_le_bytes(pqta.data[64..72].try_into().unwrap());
                println!(
                    "pool_quote_token_account balance = {balance} (REAL_SOL_RESERVES - POOL_MIGRATION_FEE = {})",
                    REAL_SOL_RESERVES - POOL_MIGRATION_FEE
                );
            }
        }
        Err(e) => {

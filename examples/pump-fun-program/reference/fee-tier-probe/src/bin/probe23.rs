//! Re-tests the original `BuybackFeeRecipientMissing` mismatch scenario,
//! now that the real blocker (missing `bonding_curve_v2`) is fixed. Every
//! prior successful probe (`probe16`, `probe19`) set ALL 8 of
//! `Global.buyback_fee_recipients` to the SAME vault as the one passed in
//! the ix — meaning a real whitelist-membership check could have been
//! silently passing this whole time without us noticing. This probe passes
//! a structurally-VALID `BuybackVault` (real seeds, real owner, real
//! discriminator, at index 0) while `Global.buyback_fee_recipients` is set
//! to 8 copies of a DIFFERENT vault (index 1) — isolating whether the real
//! check is pure PDA/seeds validity, or ALSO requires membership in
//! `Global.buyback_fee_recipients`.
...
fn global_account_data(fee_recipient: &Pubkey, buyback_official: &Pubkey) -> Vec<u8> {
    ...
    data.push(0);
    for _ in 0..8 {
        data.extend_from_slice(buyback_official.as_ref()); // ALL 8 slots = index 1's vault (NOT what we pass)
    }
...
fn main() {
    ...
    // Index 0: what we PASS in the ix (structurally valid: real seeds, real
    // owner, real discriminator).
    let (vault_index_0, _) = Pubkey::find_program_address(&[b"buyback-vault", &[0u8]], &pump_fees_program);
    svm.set_account(
        vault_index_0,
    ...
    // Index 1: what's stored in ALL 8 slots of Global.buyback_fee_recipients
    // (i.e. the "official" list does NOT include vault_index_0 at all).
    let (vault_index_1, _) = Pubkey::find_program_address(&[b"buyback-vault", &[1u8]], &pump_fees_program);
    svm.set_account(
        vault_index_1,
        Account { lamports: 10_000_000, data: buyback_vault_account_data(&Pubkey::new_unique()), owner: pump_fees_program, executable: false, rent_epoch: 0 },
    )
    ...
    println!("Passing vault_index_0 = {vault_index_0} as the ix's buyback_fee_recipient account.");
    println!("Global.buyback_fee_recipients[8] = 8 copies of vault_index_1 = {vault_index_1} (does NOT include vault_index_0).\n");

    let (global, _) = Pubkey::find_program_address(&[b"global"], &pump_program);
    svm.set_account(
        global,
        Account { lamports: 10_000_000, data: global_account_data(&fee_recipient.pubkey(), &vault_index_1), owner: pump_program, executable: false, rent_epoch: 0 },
    )
    .unwrap();
    ...
    svm.set_account(
        ...
        Account {
            lamports: 10_000_000,
            data: bonding_curve_account_data(VIRTUAL_TOKEN_RESERVES, VIRTUAL_SOL_RESERVES, REAL_TOKEN_RESERVES, TOKEN_TOTAL_SUPPLY, &creator),
            owner: pump_program,
            executable: false,
            rent_epoch: 0,
        },
    ...
    let ix = Instruction {
        ...
        accounts: vec![
            ...
            AccountMeta::new(vault_index_0, false), // structurally valid, but NOT in Global's list
        ],
        data,
    };
    ...
    match svm.send_transaction(tx) {
        Ok(_) => println!("RESULT: SUCCESS -> real check is PURE PDA/seeds+owner validity, NOT membership in Global.buyback_fee_recipients."),
        Err(e) => {
            println!("RESULT: FAILED: {:?}", e.err);
            for line in &e.meta.logs {
                println!("  LOG: {line}");
            }
            println!("\n=> If BuybackFeeRecipientMissing: real check ALSO requires membership in Global.buyback_fee_recipients — our current seeds-only design is wrong.");
        }
    }
}

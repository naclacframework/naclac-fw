//! Tests `pump_amm::create_pool` with `base_amount_in = 0` — the exact
//! scenario `pump::migrate` hits when called on a *fully graduated* bonding
//! curve (confirmed via `pump-bonding-curve/programs/pump/src/instructions/buy.rs`:
//! `associated_bonding_curve`'s real token balance tracks `real_token_reserves`
//! 1:1, both hitting exactly `0` at full graduation). Our own
//! `pool_math::lp_bootstrap_amount` computes `isqrt(0 * quote) - 100`, which
//! underflows (`0 - 100`) — this probe checks whether the REAL, deployed
//! `pump_amm.so` hits the same wall, or handles a zero base amount via some
//! different path, before assuming our formula needs to change.
...
fn main() {
    ...
    const BASE_AMOUNT_IN: u64 = 0;
    const QUOTE_AMOUNT_IN: u64 = 84_984_999_999;
    ...
    println!(
        "expected_initial_liquidity (isqrt(0)) = {expected_initial_liquidity} \
         — subtracting LP_BOOTSTRAP_WITHHELD ({LP_BOOTSTRAP_WITHHELD}) from this would underflow; \
         not doing that locally, just observing what the real program does instead."
    );

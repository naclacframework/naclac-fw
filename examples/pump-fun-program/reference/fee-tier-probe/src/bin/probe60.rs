//! Follow-up to `probe57.rs`/`probe58.rs`/`probe59.rs`, all three of which
//! DISPROVED their hypotheses: `virtual_token_reserves == 0`,
//! `real_token_reserves == 0`, and `real_quote_reserves == 0` each tested
//! alone all let the real program run its full CPI sequence (`probe59`
//! even proceeded all the way into `pump_amm::create_pool`, which then
//! failed downstream on its own `ZeroQuoteAmount` check — a genuinely
//! different failure, not the "already migrated" no-op path at all).
//!
//! This probe tests the last individual reserve-field candidate:
//! `virtual_quote_reserves == 0` alone — `virtual_token_reserves`/
//! `real_token_reserves`/`real_quote_reserves` all left genuinely nonzero,
//! no prior migration ever ran. If this too fails to trigger the no-op,
//! none of the four reserve fields alone is the check, and it's either a
//! compound condition (e.g. all four zero together) or reads something
//! else entirely (e.g. whether `pool` already has data).
//!
//! Usage: cargo run --bin probe60
...
fn main() {
    ...
    // `virtual_quote_reserves` alone is pre-zeroed, while
    // `virtual_token_reserves`/`real_token_reserves`/`real_quote_reserves`
    // stay genuinely nonzero. If real `pump.so` still treats this as
    // "already migrated" and no-ops, that isolates `virtual_quote_reserves`
    ...
    svm.set_account(
        ...
        Account {
            ...
            data: bonding_curve_account_data(&creator, true, VIRTUAL_TOKEN_RESERVES, 0, REAL_TOKEN_RESERVES, REAL_SOL_RESERVES),
            owner: pump_program,
            executable: false,
            rent_epoch: 0,
        },
    ...
    run_migrate(
        ...
        &user,
        &ix,
        "SINGLE migrate call (virtual_quote_reserves=0 only, everything else genuinely nonzero, no prior migration ever ran)",
    );
    ...
    println!("directly isolates virtual_quote_reserves==0 as the real check -- not an");
    println!("artifact of a real migration's combined effect, since virtual_token_reserves/");
    println!("real_token_reserves/real_quote_reserves were all left genuinely nonzero");
    println!("here and pool/lp_mint/etc. never existed to begin with. If it instead");
    println!("proceeded into real CPIs (and likely failed on inconsistent state), none of");
    println!("the four reserve fields alone is the check -- it's either a compound");
    println!("condition or reads something else entirely.");
}

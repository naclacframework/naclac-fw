//! Same technique as `probe42.rs`, for `sell_v2` on a non-SOL quote mint
//! (real USDC, confirmed live `Global.whitelisted_quote_mints[0]`) — real
//! deployed `pump.so` + `pump_fees.so` bytecode, executed directly in
//! litesvm. Resolves: does `sell_v2` also pass `market_cap_lamports = 0` to
//! `get_fees`? Is the fee-transfer order/direction (proceeds flow OUT of the
//! curve, fees deducted before payout) what's expected? Same cashback/
//! `sharing_config`/`associated_creator_vault` questions as `buy_v2`, now for
//! the sell direction.
//!
//! Usage: cargo run --bin probe43 [-- <rpc-url>]
...
fn run_case(client: &RpcClient, label: &str, is_cashback_coin: bool) {
    ...
    // Real quote-side reserves so the curve can actually pay out USDC.
    let real_quote_reserves = 5_000_000_000u64; // 5,000 USDC
    let mut bc_data = bonding_curve_account_data(virtual_token_reserves, virtual_quote_reserves, 793_100_000_000_000, real_quote_reserves, token_total_supply, &creator, &quote_mint);
    if is_cashback_coin {
        bc_data[82] = 1;
    }
    ...
    svm.set_account(
        ...
        Account { lamports: 2_039_280, data: token_account_data(&base_mint, &bonding_curve, 0), owner: base_token_program, executable: false, rent_epoch: 0 },
    )
    .unwrap();
    ...
    svm.set_account(
        ...
        // The curve needs real USDC on hand to pay the seller out of.
    ...
    svm.set_account(
        ...
        // User needs real base tokens on hand to sell.
        Account { lamports: 2_039_280, data: token_account_data(&base_mint, &user.pubkey(), 50_000_000_000), owner: base_token_program, executable: false, rent_epoch: 0 },
    )
    .unwrap();
    ...
    // Confirmed via probe42: not auto-created by the program -- pre-create it.
    svm.set_account(
    ...
    data.extend_from_slice(&10_000_000_000u64.to_le_bytes()); // amount (base tokens to sell)
    data.extend_from_slice(&0u64.to_le_bytes()); // min_sol_output (quote base units)

    // Real account order (25 accounts -- no `global_volume_accumulator`),
    // confirmed via pump.json + `SELL.md`.
    let ix = Instruction {
    ...
    match svm.send_transaction(tx) {
        Ok(meta) => {
            ...
            if let Some(acc) = svm.get_account(&associated_quote_user) {
                println!("associated_quote_user (user's USDC received) balance: {}", u64::from_le_bytes(acc.data[64..72].try_into().unwrap()));
            }
            if let Some(acc) = svm.get_account(&associated_base_user) {
                println!("associated_base_user (user's remaining base tokens) balance: {}", u64::from_le_bytes(acc.data[64..72].try_into().unwrap()));
            }
            if let Some(acc) = svm.get_account(&associated_creator_vault) {
                ...
                println!("associated_creator_vault: exists, data_len={} token_balance={}", acc.data.len(), amount);
            } else {
                println!("associated_creator_vault: DOES NOT EXIST after trade");
            }
            if let Some(acc) = svm.get_account(&user_volume_accumulator) {
                if acc.data.len() >= 82 {
                    println!("user_volume_accumulator.cashback_earned (offset 74..82): {}", u64::from_le_bytes(acc.data[74..82].try_into().unwrap()));
                }
            }
            if let Some(acc) = svm.get_account(&associated_user_volume_accumulator) {
                let amount = if acc.data.len() >= 72 { u64::from_le_bytes(acc.data[64..72].try_into().unwrap()) } else { 0 };
                println!("associated_user_volume_accumulator: token_balance={}", amount);
            }
            if let Some(acc) = svm.get_account(&associated_quote_fee_recipient) {
                let amount = if acc.data.len() >= 72 { u64::from_le_bytes(acc.data[64..72].try_into().unwrap()) } else { 0 };
                println!("associated_quote_fee_recipient: token_balance={}", amount);
            }
            if let Some(acc) = svm.get_account(&associated_quote_buyback_fee_recipient) {
                let amount = if acc.data.len() >= 72 { u64::from_le_bytes(acc.data[64..72].try_into().unwrap()) } else { 0 };
                println!("associated_quote_buyback_fee_recipient: token_balance={}", amount);
            }
        }
        Err(e) => {

//! Probes the real, deployed `pump.so`'s `migrate` instruction to empirically
//! determine the real SOL/token movement mechanism before implementing it in
//! `pump-bonding-curve`. The account list alone (from
//! `reference/pump-rust-client/idls/pump.json`) shows `pool_authority_mint_account`/
//! `pool_authority_wsol_account` as intermediary ATAs owned by `pool_authority`
//! (a PDA under `pump`, not a real wallet), strongly suggesting the bonding
//! curve's reserves get moved into `pool_authority`'s own accounts (SOL
//! wrapped to WSOL) before a `create_pool` CPI signed by `pool_authority` —
//! but that sequencing, and which account actually pays for the new pool's
//! rent (`user` is a read-only signer per the IDL, so it isn't the payer),
//! isn't derivable from the account list alone and needs real execution to
//! confirm. `withdraw_authority` and `pool_authority` are both pre-funded
//! with SOL here so the transaction can succeed regardless of which one
//! turns out to be the actual payer — post-transaction lamport deltas on
//! every account below settle the question either way.
//!
//! Bonding-curve/global account layouts copied from `probe28`/`probe29`
//! (already empirically confirmed there); pump_amm `GlobalConfig` layout
//! copied from `probe14`.
...
fn main() {
    ...
    // Pre-funded with SOL under System ownership so it can act as a fee
    // payer/signer-via-CPI if that's what the real program uses it for —
    // exactly the question this probe is meant to answer.
    svm.set_account(
        pool_authority,
    ...
    println!("pool_authority lamports before = {}", svm.get_account(&pool_authority).unwrap().lamports);

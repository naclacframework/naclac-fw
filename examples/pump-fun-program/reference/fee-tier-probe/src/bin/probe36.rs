//! Extends `probe31`'s real-bytecode `migrate` harness to a fully-depleted
//! bonding curve (`real_token_reserves = 0`, matching `probe35`'s confirmed
//! real graduation state) to settle what real `migrate` actually passes as
//! `pump_amm::create_pool`'s `base_amount_in`. `probe34` proved real
//! `create_pool` rejects `base_amount_in = 0`; `probe35` proved real curves
//! do reach `real_token_reserves = 0`. Those two facts only coexist if real
//! `migrate` transfers something other than the `real_token_reserves`
//! counter — the working hypothesis is `associated_bonding_curve`'s actual
//! token balance, which (per `probe22`/`probe31`/`probe35`'s shared fixture
//! numbers) sits above `real_token_reserves` by a fixed
//! `TOKEN_TOTAL_SUPPLY - initial_real_token_reserves` amount that was never
//! part of the tradeable/tracked counter. This probe sets
//! `associated_bonding_curve`'s balance to exactly that leftover amount
//! (206_900_000_000_000) while `real_token_reserves = 0`, and checks whether
//! real `migrate` succeeds and what quantity actually lands in the new
...
// Matches probe22/probe35's shared bonding-curve fixture numbers.
...
// The leftover amount that was never part of the tradeable/tracked counter,
// per this probe's hypothesis under test.
const ASSOCIATED_BONDING_CURVE_BALANCE: u64 = TOKEN_TOTAL_SUPPLY - INITIAL_REAL_TOKEN_RESERVES;

// Post-graduation bonding curve state (matches probe35's confirmed output).
const CURRENT_VIRTUAL_TOKEN_RESERVES: u64 = INITIAL_VIRTUAL_TOKEN_RESERVES - INITIAL_REAL_TOKEN_RESERVES;
const CURRENT_VIRTUAL_SOL_RESERVES: u64 = 115_005_359_057; // probe35's post-buy vSol-equivalent ballpark, exact value unused by migrate
const CURRENT_REAL_TOKEN_RESERVES: u64 = 0;
const CURRENT_REAL_SOL_RESERVES: u64 = 85_005_359_057; // probe35's confirmed post-buy real_sol_reserves
...
fn global_account_data(withdraw_authority: &Pubkey) -> Vec<u8> {
    ...
    data.extend_from_slice(&INITIAL_VIRTUAL_TOKEN_RESERVES.to_le_bytes());
    data.extend_from_slice(&INITIAL_VIRTUAL_SOL_RESERVES.to_le_bytes());
    data.extend_from_slice(&INITIAL_REAL_TOKEN_RESERVES.to_le_bytes());
...
fn bonding_curve_account_data(creator: &Pubkey) -> Vec<u8> {
    ...
    data.extend_from_slice(&CURRENT_VIRTUAL_TOKEN_RESERVES.to_le_bytes());
    data.extend_from_slice(&CURRENT_VIRTUAL_SOL_RESERVES.to_le_bytes());
    data.extend_from_slice(&CURRENT_REAL_TOKEN_RESERVES.to_le_bytes());
    data.extend_from_slice(&CURRENT_REAL_SOL_RESERVES.to_le_bytes());
    data.extend_from_slice(&TOKEN_TOTAL_SUPPLY.to_le_bytes());
    data.push(1); // complete = true (graduated, per probe35)
...
fn main() {
    ...
    svm.set_account(
        bonding_curve,
        Account {
            lamports: 10_000_000 + CURRENT_REAL_SOL_RESERVES,
    ...
    svm.set_account(
        ...
        Account { lamports: 2_039_280, data: token_account_data(&mint, &bonding_curve, ASSOCIATED_BONDING_CURVE_BALANCE), owner: token_program, executable: false, rent_epoch: 0 },
    )
    .unwrap();
    ...
    svm.set_account(
        pool_authority,
        Account { lamports: 20_000_000_000, data: vec![], owner: system_program, executable: false, rent_epoch: 0 },
    )
    .unwrap();
    ...
    println!("--- known pubkeys / inputs ---");
    println!("INITIAL_REAL_TOKEN_RESERVES = {INITIAL_REAL_TOKEN_RESERVES}");
    println!("TOKEN_TOTAL_SUPPLY = {TOKEN_TOTAL_SUPPLY}");
    println!("ASSOCIATED_BONDING_CURVE_BALANCE (hypothesis: real base_amount_in) = {ASSOCIATED_BONDING_CURVE_BALANCE}");
    println!("bonding_curve.real_token_reserves (current) = {CURRENT_REAL_TOKEN_RESERVES}");
    println!("mint = {mint}");
    ...
    match svm.send_transaction(tx) {
        Ok(meta) => {
            println!("RESULT: SUCCESS, {} CU\n", meta.compute_units_consumed);

            println!("--- inner instructions (decoded) ---");
            for (outer_idx, inner_list) in meta.inner_instructions.iter().enumerate() {
                for inner in inner_list {
                    ...
                    let accounts: Vec<String> = ci
                        .accounts
                        .iter()
                        .map(|&i| account_keys.get(i as usize).map(|p| p.to_string()).unwrap_or_else(|| "?".to_string()))
                        .collect();
                    println!(
                        "outer[{outer_idx}] stack_height={:?} program={program} data={:02x?} accounts={accounts:?}",
                        inner.stack_height, ci.data
                    );
                }
            }

            println!("\n--- post-transaction token balances ---");
            if let Some(abc) = svm.get_account(&associated_bonding_curve) {
                println!("associated_bonding_curve data[64..72] (balance) = {:02x?}", &abc.data.get(64..72));
            }
            if let Some(pam) = svm.get_account(&pool_authority_mint_account) {
                println!("pool_authority_mint_account exists, data[64..72] (balance) = {:02x?}", &pam.data.get(64..72));
            } else {
                println!("pool_authority_mint_account does NOT exist post-tx");
            }
            if let Some(pbta) = svm.get_account(&pool_base_token_account) {
                let bal = u64::from_le_bytes(pbta.data[64..72].try_into().unwrap());
                println!("pool_base_token_account balance = {bal}");
            } else {
                println!("pool_base_token_account does NOT exist post-tx");
            }
            if let Some(pqta) = svm.get_account(&pool_quote_token_account) {
                let bal = u64::from_le_bytes(pqta.data[64..72].try_into().unwrap());
                println!("pool_quote_token_account balance = {bal}");
            }
        }
        Err(e) => {

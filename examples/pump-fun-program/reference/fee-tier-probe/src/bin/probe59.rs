//! Follow-up to `probe57.rs`/`probe58.rs`, both of which DISPROVED their
//! respective hypotheses: neither `virtual_token_reserves == 0` alone nor
//! `real_token_reserves == 0` alone triggers real `migrate`'s "already
//! migrated" no-op — both times the real program ran the entire CPI
//! sequence normally. Separately, fetching the real, CURRENT on-chain
//! `bonding_curve` account for 4 independent real mainnet mints (2 that
//! fully migrated, 2 that hit the "already migrated" no-op) showed all four
//! reserve fields at exactly 0 in every case — confirming zeroing is real
//! and universal, but not distinguishing which single field the "already
//! migrated" check itself reads, since a no-op curve and a freshly-migrated
//! curve look byte-identical on those four fields.
//!
//! This probe tests the remaining reserve-field candidate:
//! `real_quote_reserves == 0` alone (semantically the most natural gate —
//! "no SOL left to migrate"; `pool_quote_amount = real_quote_reserves -
//! pool_migration_fee` is the literal amount `migrate` deposits). Fixture:
//! `real_quote_reserves` pre-set to 0, `virtual_token_reserves`/
//! `virtual_quote_reserves`/`real_token_reserves` all left genuinely
//! nonzero — no prior migration ever ran.
//!
//! Usage: cargo run --bin probe59
...
fn main() {
    ...
    // `real_quote_reserves` alone is pre-zeroed, while
    ...
    // "already migrated" and no-ops, that isolates `real_quote_reserves`
    ...
    svm.set_account(
        ...
        Account {
            ...
            data: bonding_curve_account_data(&creator, true, VIRTUAL_TOKEN_RESERVES, REAL_TOKEN_RESERVES, 0),
            owner: pump_program,
            executable: false,
            rent_epoch: 0,
        },
    ...
    run_migrate(
        ...
        &user,
        &ix,
        "SINGLE migrate call (real_quote_reserves=0 only, everything else genuinely nonzero, no prior migration ever ran)",
    );
    ...
    println!("directly isolates real_quote_reserves==0 as the real check -- not an");
    println!("artifact of a real migration's combined effect, since virtual_token_reserves/");
    println!("virtual_quote_reserves/real_token_reserves were all left genuinely nonzero");
    ...
    println!("check reads something else, not real_quote_reserves.");
}

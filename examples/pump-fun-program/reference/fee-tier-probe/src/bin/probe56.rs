//! Confirms exactly how real, deployed `pump.so` handles calling `migrate`
//! a SECOND time on a curve that's already been migrated — the user
//! observed on Solscan that a repeat `migrate` transaction always succeeds
//! but visibly does nothing. Our own `migrate.rs`/`migrate_v2.rs` have no
//! such check at all (only `require!(bonding_curve.complete, ...)`), so a
//! second call today would almost certainly hard-fail inside the nested
//! `pump_amm::create_pool` CPI once `pool`/`lp_mint` already exist from the
//! first migration — this probe determines the real detection mechanism
//! (an early, pre-CPI `Ok(())` return, since the real IDL's error list has
//! no `AlreadyMigrated`/`PoolAlreadyExists` variant at all) before changing
//! anything.
//!
//! Setup verbatim from `probe31.rs` (already proved the real `migrate`
//! account list/flow works end to end) — this probe just sends the same
//! successful `migrate` transaction twice against the same accounts, with
//! `expire_blockhash` between them so the second isn't rejected as a
//! duplicate signature, and inspects the second call's own logs/CU/account
//! deltas directly.
//!
//! Usage: cargo run --bin probe56
...
fn bonding_curve_account_data(creator: &Pubkey, complete: bool, real_token_reserves: u64, real_sol_reserves: u64) -> Vec<u8> {
    let mut data = Vec::new();
...
fn main() {
    ...
    svm.set_account(
        ...
        Account {
            ...
            data: bonding_curve_account_data(&creator, true, REAL_TOKEN_RESERVES, REAL_SOL_RESERVES),
            owner: pump_program,
            executable: false,
            rent_epoch: 0,
        },
    ...
    run_migrate(&mut svm, &user, &ix, "FIRST migrate call (curve complete, not yet migrated)");

    println!("\npost-first-migrate bonding_curve data: {:02x?}", svm.get_account(&bonding_curve).unwrap().data);
    println!("post-first-migrate pool exists: {}", svm.get_account(&pool).is_some());

    svm.expire_blockhash();

    run_migrate(&mut svm, &user, &ix, "SECOND migrate call (same curve, already migrated)");

    println!("\nCONCLUSION: if the second call FAILED, real pump.so has no graceful");
    println!("already-migrated handling and our own migrate.rs/migrate_v2.rs already");
    println!("match real behavior (both would also fail on a second call). If it");
    println!("SUCCEEDED with zero inner instructions, that's the real early-return");
    println!("no-op the user observed on Solscan -- check bonding_curve's own state");
    println!("above (real_token_reserves/real_quote_reserves) for what condition");
    println!("the real handler is checking to detect 'already migrated'.");
}

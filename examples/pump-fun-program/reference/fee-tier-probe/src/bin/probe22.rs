//! Settles whether real `buy`'s `fee_recipient` account check is strict
//! equality against `global.fee_recipient`, or pool-membership against
//! `{global.fee_recipient} ∪ {global.fee_recipients[7]}` — suggested by the
//! real client SDK's `fee_recipient_from_pump_global` helper
//! (`reference/pump-rust-client/src/sdk/mod.rs`), which randomly picks from
//! that combined pool, but `probe19` Case B's mismatch test (which had an
//! all-zero pool) can't distinguish the two designs. Passes a pool entry
//! (NOT the primary `fee_recipient`) as the `fee_recipient` account.
...
// Primary fee_recipient = primary. fee_recipients[7]: entry[3] = pool_entry,
// rest zero. buyback_fee_recipients[8] = all buyback.
...
fn main() {
    ...
    println!("primary fee_recipient = {}", primary_fee_recipient.pubkey());
    println!("pool[3] fee_recipient = {} (this is what we'll pass as the ix account, NOT the primary)\n", pool_fee_recipient.pubkey());
    ...
    let ix = Instruction {
        ...
        accounts: vec![
            ...
            // Passing the POOL entry (index 3), NOT the primary fee_recipient.
            AccountMeta::new(pool_fee_recipient.pubkey(), false),
    ...
    match svm.send_transaction(tx) {
        Ok(_) => println!("RESULT: SUCCESS -> real check is POOL-MEMBERSHIP (accepts fee_recipients[7] entries too, not just the primary)"),
        Err(e) => {
            println!("RESULT: FAILED: {:?}", e.err);
            for line in &e.meta.logs {
                println!("  LOG: {line}");
            }
            println!("\n=> If NotAuthorized: real check is STRICT EQUALITY to global.fee_recipient only, pool membership is NOT accepted here.");
        }
    }
}

//! Determines the real graduation trigger: does a `buy` that consumes the
//! entire `real_token_reserves` succeed against real `pump.so`, and if so
//! what value does `real_token_reserves` end up holding? Needed because
//! `probe34` proved real `pump_amm::create_pool` rejects `base_amount_in =
//! 0`, which would be unreachable if real bonding curves actually let
//! `real_token_reserves` hit exactly zero at graduation the way this
//! framework's own `buy.rs` currently assumes.
...
fn global_account_data(primary: &Pubkey, pool_entry: &Pubkey, buyback: &Pubkey) -> Vec<u8> {
    let mut data = Vec::new();
    data.extend_from_slice(&GLOBAL_DISCRIMINATOR);
    data.push(1); // initialized
    data.extend_from_slice(&[0u8; 32]); // authority
    data.extend_from_slice(primary.as_ref()); // fee_recipient
    ...
    for i in 0..7 {
        if i == 3 {
            data.extend_from_slice(pool_entry.as_ref());
        } else {
    ...
    for _ in 0..8 {
        data.extend_from_slice(buyback.as_ref());
    }
...
fn main() {
    ...
    let primary_fee_recipient = Keypair::new();
    let pool_fee_recipient = Keypair::new();
    svm.airdrop(&primary_fee_recipient.pubkey(), 10_000_000_000).unwrap();
    svm.airdrop(&pool_fee_recipient.pubkey(), 10_000_000_000).unwrap();
    ...
    svm.set_account(
        ...
        Account {
            lamports: 10_000_000,
            data: global_account_data(&primary_fee_recipient.pubkey(), &pool_fee_recipient.pubkey(), &buyback_fee_recipient),
            owner: pump_program,
            executable: false,
            rent_epoch: 0,
        },
    ...
    // Try to buy the ENTIRE real_token_reserves in a single trade.
    ...
    data.extend_from_slice(&1_000_000_000_000u64.to_le_bytes()); // max_sol_cost
    data.push(0);
    ...
    let ix = Instruction {
        ...
        accounts: vec![
            ...
            AccountMeta::new(primary_fee_recipient.pubkey(), false),
    ...
    println!("Attempting to buy the ENTIRE real_token_reserves ({REAL_TOKEN_RESERVES}) in one trade.\n");

    match svm.send_transaction(tx) {
        Ok(_) => {
            println!("RESULT: SUCCESS -> full-reserve buy is accepted by the real program.\n");
            let post = svm.get_account(&bonding_curve).expect("bonding_curve account must still exist");
            let d = &post.data;
            let real_token_reserves_after = u64::from_le_bytes(d[24..32].try_into().unwrap());
            let real_sol_reserves_after = u64::from_le_bytes(d[32..40].try_into().unwrap());
            let complete_after = d[48];
            println!("post-buy bonding_curve.real_token_reserves = {real_token_reserves_after}");
            println!("post-buy bonding_curve.real_sol_reserves   = {real_sol_reserves_after}");
            println!("post-buy bonding_curve.complete             = {complete_after}");
        }
        Err(e) => {

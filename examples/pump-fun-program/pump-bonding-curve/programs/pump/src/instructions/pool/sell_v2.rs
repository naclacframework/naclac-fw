...
use crate::constants::{
    ...
    FEE_CONFIG_SEED, GLOBAL_SEED, PUMP_AUTHORITY_SEED, PUMP_FEES_PROGRAM_ID, USER_VOLUME_ACCUMULATOR_SEED,
    WSOL_MINT,
};
...
// Real `sell_v2` account list (26 accounts — no `global_volume_accumulator`,
// same asymmetry classic `buy`/`sell` already have) confirmed via
// `pump-public-docs/idl/pump.json` + `SELL.md`, real-behavior-verified for a
// non-SOL (USDC) quote mint via `reference/fee-tier-probe/src/bin/probe43.rs`
// — see `docs/plan/bonding-curve-05-batch1-v2-instructions.md`. Confirmed
// exact (not assumed): the bonding-curve formula is identical to classic
// `sell`'s `gross_sol_for_tokens_sell` (same rounding), and the fee-deduction
// arithmetic (gross − protocol_fee − creator_fee) matches the real observed
// payout exactly. Cashback/`sharing_config`/`associated_creator_vault`/
// `associated_quote_buyback_fee_recipient` behavior all match `buy_v2`'s
// confirmed findings (same non-SOL fund-flow conventions). For a SOL-paired
// coin (`quote_mint == WSOL_MINT`), `sell_v2` uses the exact same
// native-lamport fund flow as classic `sell.rs` — it never touches any
// quote-side WSOL ATA. Confirmed with certainty via
// `reference/fee-tier-probe/src/bin/probe51.rs` (every quote-side ATA stayed
// at balance 0 through a real `sell_v2` call against real, deployed
// `pump.so` bytecode) and independently via the real IDL's own doc comment
// on `associated_quote_user`: "ignored for legacy (SOL) trades". `creator_vault`
// gets the same conditional rent-exempt top-up `buy_v2.rs` does (see its
// module comment for how the exact threshold was confirmed), regardless of
// quote mint.
#[instruction_args]
...
pub fn sell_v2(ctx: Context<SellV2>, args: SellV2Args) -> Result {
    require!(args.amount > 0, PumpError::SellZeroAmount);
    ...
    let gross_quote = gross_sol_for_tokens_sell(
        args.amount as u128,
    ...
    let fees = get_fees_via_cpi(
        ...
        GetFeesParams {
            ...
            trade_size_lamports: gross_quote,
    ...
    let creator_fee = if ctx.accounts.bonding_curve.creator == Address::default() {
        ...
        fee_amount_ceil(gross_quote as u128, fees.creator_fee_bps)? as u64
    };
    let protocol_fee = fee_amount_ceil(gross_quote as u128, fees.protocol_fee_bps)? as u64;

    let total_fees = protocol_fee.checked_add(creator_fee).ok_or(PumpError::MathOverflow)?;
    let quote_output = gross_quote.checked_sub(total_fees).ok_or(PumpError::MathOverflow)?;
    require!(quote_output >= args.min_sol_output, PumpError::SlippageExceeded);
    ...
    let bonding_curve_signer_seeds: &[&[u8]] = &[BONDING_CURVE_SEED, base_mint_address.as_ref(), &[args.bonding_curve_bump]];
    let bonding_curve_signer: &[&[&[u8]]] = &[bonding_curve_signer_seeds];

    ctx.accounts.base_token_program.transfer_checked(
        TransferCheckedAccounts {
            from: &mut ctx.accounts.associated_base_user,
            mint: &ctx.accounts.base_mint,
            to: &mut ctx.accounts.associated_base_bonding_curve,
    ...
    // that threshold, and only when it's the real destination (not a
    // cashback coin) — see `buy_v2.rs`'s module comment for how this was
    // confirmed exactly. Applies regardless of quote mint.
    ...
    if is_sol_quote {
        // Native-lamport path -- same fund flow as classic `sell.rs`
        // (bonding_curve/creator_vault/user_volume_accumulator are all this
        // program's own accounts, so raw lamport debits/credits need no CPI).
        // See module comment: confirmed with certainty this is the real
        // behavior, not an SPL transfer into `associated_quote_user`.
        ctx.accounts.bonding_curve.sub_lamports(gross_quote)?;
        ctx.accounts.user.add_lamports(quote_output)?;
        ...
        // Real SPL-transfer path, confirmed via probe: the curve's own
        // quote ATA is the source, signed by `bonding_curve`.
    ...
    bonding_curve.virtual_quote_reserves =
        bonding_curve.virtual_quote_reserves.checked_sub(gross_quote).ok_or(PumpError::MathOverflow)?;
    ...
    bonding_curve.real_quote_reserves =
        bonding_curve.real_quote_reserves.checked_sub(gross_quote).ok_or(PumpError::MathOverflow)?;

...
unsafe impl crate::sdk_core_offchain::bytemuck::Zeroable for InitUserVolumeAccumulatorIxArgs {}
#[cfg(feature = "offchain")]
#[cfg(not(feature = "borsh"))]
unsafe impl crate::sdk_core_offchain::bytemuck::Pod for InitUserVolumeAccumulatorIxArgs {}
...
#[cfg_attr(not(feature = "borsh"), derive(Copy, Clone, Debug))]
#[cfg_attr(not(feature = "borsh"), repr(C, packed))]
pub struct InitUserVolumeAccumulatorCpiIxArgs {
    pub user_volume_accumulator_bump: u8,
}

#[cfg(feature = "cpi")]
#[cfg(not(feature = "borsh"))]
unsafe impl crate::sdk_core_cpi::bytemuck::Zeroable for InitUserVolumeAccumulatorCpiIxArgs {}
#[cfg(feature = "cpi")]
#[cfg(not(feature = "borsh"))]
unsafe impl crate::sdk_core_cpi::bytemuck::Pod for InitUserVolumeAccumulatorCpiIxArgs {}
...
/// funded by `payer` -- permissionless, matching the real program's own
/// account list (no signer requirement on `user` at all).
pub fn build_init_user_volume_accumulator<'a>(
    ...
    let mut ix_data = crate::sdk_core_offchain::vec![94, 6, 202, 115, 255, 96, 232, 183];
    let args = InitUserVolumeAccumulatorIxArgs {
...
/// funded by `payer` -- permissionless, matching the real program's own
/// account list (no signer requirement on `user` at all).
pub struct InitUserVolumeAccumulatorCpiAccounts<'a> {
...
#[cfg(feature = "cpi")]
pub struct InitUserVolumeAccumulatorCpiCall<'a> {
    pub accounts: InitUserVolumeAccumulatorCpiAccounts<'a>,
    pub user_volume_accumulator_bump: u8,
...
#[cfg(feature = "cpi")]
pub trait InitUserVolumeAccumulatorCpi<'info> {
    /// Creates a brand-new, zeroed `UserVolumeAccumulator` PDA for `user`,
    /// funded by `payer` -- permissionless, matching the real program's own
    /// account list (no signer requirement on `user` at all).
    fn init_user_volume_accumulator<'a>(
        &self,
        accounts: InitUserVolumeAccumulatorCpiAccounts<'a>,
        user_volume_accumulator_bump: u8
    ) -> crate::sdk_core_cpi::Result<()> {
        self.init_user_volume_accumulator_with_remaining_accounts(InitUserVolumeAccumulatorCpiCall {
        accounts,
        user_volume_accumulator_bump,
        signer_seeds: &[],
        remaining_accounts: &[],
    ...
    fn init_user_volume_accumulator_signed<'a>(
        &self,
        accounts: InitUserVolumeAccumulatorCpiAccounts<'a>,
        user_volume_accumulator_bump: u8,
        signer_seeds: &[&[&[u8]]]
    ...
    fn init_user_volume_accumulator_with_remaining_accounts<'a>(
        &self,
        call: InitUserVolumeAccumulatorCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()>;

}

#[cfg(feature = "cpi")]
impl<'info> InitUserVolumeAccumulatorCpi<'info> for crate::sdk_core_cpi::Program<crate::Pump> {
    fn init_user_volume_accumulator_with_remaining_accounts<'a>(
        &self,
        call: InitUserVolumeAccumulatorCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        let InitUserVolumeAccumulatorCpiCall { accounts, user_volume_accumulator_bump, signer_seeds, remaining_accounts } = call;
        let args = InitUserVolumeAccumulatorCpiIxArgs {
            user_volume_accumulator_bump,
        };
        let mut ix_data = crate::sdk_core_cpi::Vec::new();
        ix_data.extend_from_slice(&[94, 6, 202, 115, 255, 96, 232, 183]);
        #[cfg(not(feature = "borsh"))]
        {

...
import { PumpFeesClient } from "../clients/src/generated/pump_fees";

describe("Naclac PumpFees Test Suite", () => {
  let client: PumpFeesClient;
  let payer: naclac.KeyPairSigner;
  let counterPda: naclac.Address;

  before(async () => {
    payer = await naclac.loadNodeWallet();
    client = new PumpFeesClient("localnet", payer);

...
// `authority` must equal `sharing_config.admin` — confirmed empirically via
// `reference/fee-tier-probe/src/bin/probe12.rs` against the real bytecode:
// signing with `sharing_config.admin` (and a deliberately different
// `global.admin_set_creator_authority`) passes the check and reaches the CPI
// chain; signing with `global.admin_set_creator_authority` instead fails
// immediately with `NotAuthorized`, before any CPI runs. So despite sharing
// `reset_fee_sharing_config`'s payout handler, `update_fee_shares(_v2)` does
// NOT share its authorization source.
#[derive(Accounts)]

...
pub fn build_transfer_fee_sharing_authority<'a>(
    provider: &'a naclac_client::NaclacProvider,
    program_id: naclac_client::Address,
) -> naclac_client::InstructionBuilder<'a> {
    let ix_data = crate::sdk_core_offchain::vec![202, 10, 75, 200, 164, 34, 210, 96];
    naclac_client::InstructionBuilder::new(provider, program_id, ix_data)
}

#[cfg(feature = "cpi")]
pub struct TransferFeeSharingAuthorityCpiCall<'a> {
    pub signer_seeds: &'a [&'a [&'a [u8]]],
    pub remaining_accounts: &'a [(crate::sdk_core_cpi::AccountInfo, bool, bool)],
}

#[cfg(feature = "cpi")]
pub trait TransferFeeSharingAuthorityCpi<'info> {
    /// Transfer Fee Sharing Authority
    /// 
    /// confirmed via direct execution against the real bytecode —
    /// this instruction always reverts with DeprecatedInstruction, checked
    /// before touching any accounts.
    fn transfer_fee_sharing_authority<'a>(
        &self,
        
    ) -> crate::sdk_core_cpi::Result<()> {
        self.transfer_fee_sharing_authority_with_remaining_accounts(TransferFeeSharingAuthorityCpiCall {
        signer_seeds: &[],
        remaining_accounts: &[],
    })
    }

    fn transfer_fee_sharing_authority_signed<'a>(
        &self,
        signer_seeds: &[&[&[u8]]]
    ) -> crate::sdk_core_cpi::Result<()> {
        self.transfer_fee_sharing_authority_with_remaining_accounts(TransferFeeSharingAuthorityCpiCall {
    ...
    fn transfer_fee_sharing_authority_with_remaining_accounts<'a>(
        &self,
        call: TransferFeeSharingAuthorityCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()>;

}

#[cfg(feature = "cpi")]
impl<'info> TransferFeeSharingAuthorityCpi<'info> for crate::sdk_core_cpi::Program<crate::PumpFees> {
    fn transfer_fee_sharing_authority_with_remaining_accounts<'a>(
        &self,
        call: TransferFeeSharingAuthorityCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        let TransferFeeSharingAuthorityCpiCall { signer_seeds, remaining_accounts } = call;
        let mut ix_data = crate::sdk_core_cpi::Vec::new();
        ix_data.extend_from_slice(&[202, 10, 75, 200, 164, 34, 210, 96]);

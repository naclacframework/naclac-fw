...
pub fn build_revoke_fee_sharing_authority<'a>(
    provider: &'a naclac_client::NaclacProvider,
    program_id: naclac_client::Address,
) -> naclac_client::InstructionBuilder<'a> {
    let ix_data = crate::sdk_core_offchain::vec![18, 233, 158, 39, 185, 207, 58, 104];
    naclac_client::InstructionBuilder::new(provider, program_id, ix_data)
}

#[cfg(feature = "cpi")]
pub struct RevokeFeeSharingAuthorityCpiCall<'a> {
    pub signer_seeds: &'a [&'a [&'a [u8]]],
    pub remaining_accounts: &'a [(crate::sdk_core_cpi::AccountInfo, bool, bool)],
}

#[cfg(feature = "cpi")]
pub trait RevokeFeeSharingAuthorityCpi<'info> {
    /// Revoke Fee Sharing Authority
    /// 
    /// confirmed via direct execution against the real bytecode —
    /// this instruction always reverts with DeprecatedInstruction, checked
    /// before touching any accounts.
    fn revoke_fee_sharing_authority<'a>(
        &self,
        
    ) -> crate::sdk_core_cpi::Result<()> {
        self.revoke_fee_sharing_authority_with_remaining_accounts(RevokeFeeSharingAuthorityCpiCall {
        signer_seeds: &[],
        remaining_accounts: &[],
    })
    }

    fn revoke_fee_sharing_authority_signed<'a>(
        &self,
        signer_seeds: &[&[&[u8]]]
    ) -> crate::sdk_core_cpi::Result<()> {
        self.revoke_fee_sharing_authority_with_remaining_accounts(RevokeFeeSharingAuthorityCpiCall {
    ...
    fn revoke_fee_sharing_authority_with_remaining_accounts<'a>(
        &self,
        call: RevokeFeeSharingAuthorityCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()>;

}

#[cfg(feature = "cpi")]
impl<'info> RevokeFeeSharingAuthorityCpi<'info> for crate::sdk_core_cpi::Program<crate::PumpFees> {
    fn revoke_fee_sharing_authority_with_remaining_accounts<'a>(
        &self,
        call: RevokeFeeSharingAuthorityCpiCall<'a>
    ) -> crate::sdk_core_cpi::Result<()> {
        let RevokeFeeSharingAuthorityCpiCall { signer_seeds, remaining_accounts } = call;
        let mut ix_data = crate::sdk_core_cpi::Vec::new();
        ix_data.extend_from_slice(&[18, 233, 158, 39, 185, 207, 58, 104]);

...
import { PumpAmmClient } from "../clients/src/generated/pump_amm";

describe("Naclac PumpAmm Test Suite", () => {
  let client: PumpAmmClient;
  let payer: naclac.KeyPairSigner;
  let counterPda: naclac.Address;

  before(async () => {
    payer = await naclac.loadNodeWallet();
    client = new PumpAmmClient("localnet", payer);

...
import { DonationRelayClient } from "../clients/src/generated/donation_relay";

describe("Naclac DonationRelay Test Suite", () => {
  let client: DonationRelayClient;
  let payer: naclac.KeyPairSigner;
  let counterPda: naclac.Address;

  before(async () => {
    payer = await naclac.loadNodeWallet();
    client = new DonationRelayClient("localnet", payer);

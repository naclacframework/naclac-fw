...
import { LaunchpadBorshClient } from "../clients/src/generated/launchpad_borsh";

describe("Naclac LaunchpadBorsh Test Suite", () => {
  let client: LaunchpadBorshClient;
  let payer: naclac.KeyPairSigner;
  let counterPda: naclac.Address;

  before(async () => {
    payer = await naclac.loadNodeWallet();
    client = new LaunchpadBorshClient("localnet", payer);

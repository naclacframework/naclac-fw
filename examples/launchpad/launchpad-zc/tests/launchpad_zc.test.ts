...
import { LaunchpadZcClient } from "../clients/src/generated/launchpad_zc";

describe("Naclac LaunchpadZc Test Suite", () => {
  let client: LaunchpadZcClient;
  let payer: naclac.KeyPairSigner;
  let counterPda: naclac.Address;

  before(async () => {
    payer = await naclac.loadNodeWallet();
    client = new LaunchpadZcClient("localnet", payer);
